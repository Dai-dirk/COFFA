...
def run(*argv):
    ...
    x4 = LpVariable("x4", lowBound=1, upBound=1, cat='Integer')
    x5 = LpVariable("x5", lowBound=None, upBound=None, cat='Integer')
    ...
    problem += z2 * int(argv[9]) <= phi_2[0]*x1 + phi_2[1]*x2 + phi_2[2]*x3 + phi_2[3]*x4
    problem += (z1 + 1) * int(argv[9]) - (phi_1[0]*x1 + phi_1[1]*x2 + phi_1[2]*x3 + phi_1[3]*x4) >= 1
    problem += (z2 + 1) * int(argv[9]) - (phi_2[0]*x1 + phi_2[1]*x2 + phi_2[2]*x3 + phi_2[3]*x4) >= 1
...
if __name__ == "__main__":
    run(0,0,0,1,0,0,0,0,1,4,0,29,0,29,0,29)
    sys.exit(1)
    if len(sys.argv) != 12:
