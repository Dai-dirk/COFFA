...
// Forward declarations for helper predicates used before their definitions
static bool isFloatValueNode(LLVMCDFGNode* node);

// Format constant for DOT:
//  - fp32/fp64 constants are shown as decimal numbers (decoded from bit patterns)
//  - integer constants remain decimal
static std::string formatConstForNode(LLVMCDFGNode* n){
    ...
    if(n->dataBits() == 32){
        ...
        oss << std::fixed << std::setprecision(std::numeric_limits<float>::max_digits10) << f;
        return oss.str();
...
LLVMCDFGNode* LLVMCDFG::addNode(Instruction *ins)
{
    ...
    if(dyn_cast<LoadInst>(ins)){
        ...
        _arrayName2LSNode[node->getLSArrayName()].first.insert(node);
    }else if(dyn_cast<StoreInst>(ins)){
        _storeList.insert(node);
        _arrayName2LSNode[node->getLSArrayName()].second.insert(node);
    }
    int id = 0;
    if(!_nodes.empty()){
...
void LLVMCDFG::bypassBitcastNode(LLVMCDFGNode *node)
{
    if(node == nullptr){
        return;
    }
    auto inputs = node->inputNodes();
    if(inputs.size() != 1){
        node->setFinalInstruction("BitCast");
        return;
    }
    LLVMCDFGNode *inputNode = inputs.front();
    ...
    for(auto outNode : outputs){
        ...
        CondVal cond = outNode->getInputCondVal(node);
        LLVMCDFGEdge *bridgeEdge = edge(node, outNode);
        ...
        inputNode->addOutputNode(outNode, isBackEdge, cond);
        inputNode->setOutputBackEdge(outNode, isBackEdge);
        inputNode->setOutputCondVal(outNode, cond);
        outNode->addInputNode(inputNode, idx, isBackEdge, cond);
        outNode->setInputIdx(inputNode, idx);
        outNode->setInputBackEdge(inputNode, isBackEdge);
        outNode->setInputCondVal(inputNode, cond);
        LLVMCDFGEdge *newEdge = edge(inputNode, outNode);
        if(!newEdge){
            newEdge = addEdge(inputNode, outNode, edgeType);
        }
        if(newEdge){
...
void LLVMCDFG::initialize()
{
    ...
    //FIXME: to handle the _taskIOBBs when loopsafetycheck
    // for(auto BB : _taskIOBBs){
    //     allBBs.insert(BB);
    // }
    outs() << ">>>>>>>>>>>>>allBBs: ";
    for(auto bb : allBBs){
        ...
        for(auto &I : *BB){
            ...
            if(!dyn_cast<PHINode>(ins)){
                ...
                for(Use &pred : ins->operands()){
                    ...
                    if(ConstantInt *CI = dyn_cast<ConstantInt>(pred)){
                        ...
                        int constant = 0;
                        if(FP->getType()->getTypeID() == Type::DoubleTyID){
                            constant = (DataType)FP->getValueAPF().convertToDouble();
                        }else if(FP->getType()->getTypeID() == Type::FloatTyID){
                            constant = (DataType)FP->getValueAPF().convertToFloat();
                        }else{
                            assert(false && "bad FP type");
                        }
                        inputNode = addNode("CONST", node->BB());
                    ...
                    }else if (auto GV = dyn_cast<GlobalValue>(pred)){
                        // Treat global variables/constants as INPUT nodes when they are real data operands.
                        // For CallInst, the callee (a GlobalValue Function) must NOT be wired as a data input.
                        if (auto CI = dyn_cast<CallInst>(ins)){
                            Value* callee = CI->getCalledOperand();
                            if (GV == callee) {
                                // skip adding an input edge for the callee
                                // idx++ happens at the end of the loop
                                // no inputNode assignment means no edge will be created
                            } else {
                                inputNode = getInputNode(GV, BB);
                            }
                        } else {
                            inputNode = getInputNode(GV, BB);
                        }
            ...
            for(User *succ : ins->users()){               
                if(Instruction *succIns = dyn_cast<Instruction>(succ)){
                    ...
                    if(!allBBs.count(succIns->getParent()) || _ExInsList.count(succIns)){ // out of loop BB
                        errs() << "[BugCheck]: " << "current ins: "; ins->dump();
                        errs() << "[BugCheck]: " << "succ ins: "; succIns->dump();
                        succNode = getOutputNode(succIns, BB);
                        succNode->setInstruction(succIns);
    ...
    for(auto &elem : temnode){
        ...
        if(node->customInstruction() == "OUTPUT"){
            errs() << node->getName(); node->instruction()->dump();
            assert(node->inputNodes().size() == 1 && "OUTPUT has only 1 input");
            // auto result = node->inputNodes()[0];
            // Instruction* resultins = result->instruction();
            // assert(resultins != NULL);
            /// TODO: how to do in multiTask
...
LLVMCDFGNode* LLVMCDFG::getInputNode(Value *ins, BasicBlock *BB)
{
    ...
    // If it's a pointer-like global/arg, record it so that getLSArrayName() works for pretty printing
    if (ins->getType()->isPointerTy()){
        node->setLSaddress(ins);
    }
    return node;
}
...
*/
	...
	for(LLVMCDFGNode* node : phiNodes){
        ...
        // std::cout << "node out size: " << node->outputNodes().size() << std::endl;
        // printDOT(std::to_string(printTimes) + "_" + _name + "_in_handlephi.dot");
        for(auto succ : node->outputNodes()){
            bool isBackEdge = (succ->isInputBackEdge(node) | lastNode->isInputBackEdge(node));
            int idx;
            /*
            @yuan: I don't know why when the phi node has two same output edges connect to
                   the same node, but only one edge is record, but I fix it here 
            */
            // std::cout << "succ instruction: " << succ->getName() << " innode size: " << succ->inputNodes().size()<< std::endl;
            ...
            // std::cout << "addedNodePortPairs size: " << addedNodePortPairs.size() << std::endl;
            ...
            if(sameInput > 1){
                for(auto& allInput : addedNodePortPairs){
                    if(allInput.first != succ) continue;
                    lastNode->addOutputNode(succ, isBackEdge);
                    succ->addInputNode(lastNode, allInput.second, isBackEdge);
...
void LLVMCDFG::handleConversionNodes()
{
    for(auto &elem : _nodes){
        auto node = elem.second;
        std::string finalInstr = node->finalInstruction();
        ...
        if(ins){
            switch (ins->getOpcode()){
                ...
                case Instruction::ZExt:{
                    ...
                    // constNode->addOutputNode(node);      
                    // addEdge(constNode, node, EDGE_TYPE_DATA);  
                    auto ZExtIn = node->getInputPort(0);
                    delEdge(edge(ZExtIn, node));
                    auto outputs = node->outputNodes();
                    for(auto output : outputs){
                        int idx = output->getInputIdx(node);
                        delEdge(edge(node, output));
                        connectNodes(ZExtIn, output, idx, EDGE_TYPE_DATA);
                    }
                    break;
                }
                #endif
                //for ZQL: to do calculation of address
                case Instruction::PtrToInt:{
                    auto PtrToIntIn = node->getInputPort(0);
                    delEdge(edge(PtrToIntIn, node));
                    auto outputs = node->outputNodes();
                    for(auto output : outputs){
                        int idx = output->getInputIdx(node);
                        delEdge(edge(node, output));
                        connectNodes(PtrToIntIn, output, idx, EDGE_TYPE_DATA);
                    }
                    break;
                }
                default: break;
...
{
    std::cout << "fuseAccConstNode~~~~~~~~~~~~~~~ _nodes size: " << _nodes.size() << std::endl;
    auto nodes = _nodes;
    for(auto &elem : nodes){
        auto node = elem.second;
        std::cout << "instruction: " << node->customInstruction() << " out size: " << node->outputNodes().size()<< std::endl;
        if(node->isAcc() && node->outputNodes().size() == 1){
            auto op0 = node->getInputPort(0);
            std::cout << "out instruction: " << op0->customInstruction() << std::endl;
                ...
                if(isMul || (isShl && dst->getInputIdx(node) == 0)){
                    ...
                    if(otherOp->customInstruction() == "CONST"){
                        ...
                        if(isMul){
                            ...
                            node->setInitial(node->getInitial() * otherConstVal);
                        }else{
                            accConstVal <<= otherConstVal;
                            node->setInitial(node->getInitial() << otherConstVal);
                        }
...
// fold (0 - x) or -x feeding const mul/div by absorbing the negation into the constant
void LLVMCDFG::fuseZeroSubConstOps()
{
    auto isZeroConst = [](LLVMCDFGNode *node) -> bool {
        if(!node || !node->hasConst()) return false;
        int bits = node->dataBits();
        if(bits == 32){
            uint32_t raw = static_cast<uint32_t>(node->constVal());
            float f;
            std::memcpy(&f, &raw, sizeof(float));
            return f == 0.0f;
        }else if(bits == 64){
            uint64_t raw = static_cast<uint64_t>(node->constVal());
            double d;
            std::memcpy(&d, &raw, sizeof(double));
            return d == 0.0;
        }else{
            return static_cast<int64_t>(node->constVal()) == 0;
        }
    };

    auto negateConst = [](LLVMCDFGNode *node) -> bool {
        if(!node || !node->hasConst()){
            return false;
        }
        int bits = node->dataBits();
        if(bits == 32){
            ...
            int64_t v = static_cast<int64_t>(node->constVal());
            node->setConstVal(static_cast<DataType>(-v));
            return true;
        }
    };

    std::vector<LLVMCDFGNode*> removeNodes;
    auto nodes = _nodes;
    for(auto &elem : nodes){
        auto node = elem.second;
        Instruction *ins = node->instruction();
        LLVMCDFGNode *valueNode = nullptr;
        LLVMCDFGNode *zeroNode = nullptr;
        if(ins){
            auto opc = ins->getOpcode();
            if(opc == Instruction::FNeg){
                valueNode = node->getInputPort(0);
            }else if(opc == Instruction::FSub){
                auto lhs = node->getInputPort(0);
                auto rhs = node->getInputPort(1);
                if(isZeroConst(lhs)){
                    zeroNode = lhs;
                    valueNode = rhs;
                }
        ...
        if(!valueNode){
            auto custom = node->customInstruction();
            auto final = node->finalInstruction();
            if(custom == "FSUB" || final == "FSUB"){
                auto lhs = node->getInputPort(0);
                auto rhs = node->getInputPort(1);
                if(isZeroConst(lhs)){
                    zeroNode = lhs;
        ...
        const auto &outs = node->outputNodes();
        if(outs.size() != 1){
            continue;
        }
        auto consumer = outs.front();
        if(!consumer){
            continue;
        }
        Instruction *cIns = consumer->instruction();
        std::string customIns = consumer->customInstruction();
        bool isMul = false;
        bool isDiv = false;
        if(cIns){
            if(cIns->getOpcode() == Instruction::FMul){
                isMul = true;
            }else if(cIns->getOpcode() == Instruction::FDiv){
                isDiv = true;
            }
        }
        if(!isMul && !isDiv){
        ...
        int idx = consumer->getInputIdx(node);
        if(idx < 0){
            continue;
        }
        if(isDiv && idx != 0){
        ...
        auto constNode = consumer->getInputPort(otherIdx);
        if(!constNode){
            continue;
        }
        if(!constNode->hasConst()){
            continue;
        }
        if(!negateConst(constNode)){
            continue;
        }
        bool isBackEdge = consumer->isInputBackEdge(node);
        CondVal cond = consumer->getInputCondVal(node);
        auto bridge = edge(node, consumer);
        if(bridge){
            delEdge(bridge);
        }else{
            consumer->delInputNode(node);
            node->delOutputNode(consumer);
        }
        consumer->addInputNode(valueNode, idx, isBackEdge, cond);
        valueNode->addOutputNode(consumer, isBackEdge, cond);
        addEdge(valueNode, consumer, EDGE_TYPE_DATA);

        removeNodes.push_back(node);
        if(zeroNode && zeroNode->outputNodes().empty()){
            delNode(zeroNode);
        }
    }

    for(auto node : removeNodes){
        if(node && node->outputNodes().empty()){
            delNode(node);
        }
    }
}
...
void LLVMCDFG::handleGEPNodes()
{
    ...
    for(auto node : GEPNodes){
        ...
        if(node->customInstruction() == "OUTPUT"){
            errs() << "the GEP is an outter instruction, continue\n";
            continue;
        }
        // Special-case: base pointer is a SELECT between two arrays.
        // For CGRA DFG, we cannot do address ops; rewrite to value SELECT:
        //   load(select(A_ptr, B_ptr)[idx])  =>  select(load(A[idx]), load(B[idx]))
        if (auto GEP = dyn_cast<GetElementPtrInst>(node->instruction())){
            Value* basePtr = GEP->getPointerOperand();
            if (auto selIns = dyn_cast<SelectInst>(basePtr)){
                // DFG node for this SELECT
                auto selNode = this->node(selIns);
                if(selNode && selNode->customInstruction() == "SELECT"){
                    // Identify the two data parents of the SELECT (expected INPUT nodes for arrays)
                    auto selOp0 = selNode->getInputPort(0);
                    auto selOp1 = selNode->getInputPort(1);
                    // Compute affine pattern from this GEP for later use by the array INPUTs
                    // element size should be the final indexed type of this GEP (i.e., the loaded element)
                    int elemsize = DL->getTypeAllocSize(GEP->getResultElementType());
                    // naive affine stride/bounds: stride only on innermost loop
                    std::map<int, varType> LSstride; for(int i=0;i<getLoopsAffineStrides().size();i++){ LSstride[i]=0; }
                    varType LSbounds[3]; LSbounds[0]=0; LSbounds[1]=0; LSbounds[2]=elemsize;
                    if(!getLoopsAffineCounts().empty()){
                        LSstride[0] = elemsize;
                        LSbounds[1] = (getLoopsAffineCounts(0) - 1) * elemsize;
                    }
                    // annotate two array INPUT nodes (if present) with affine info
                    auto annotateInput = [&](LLVMCDFGNode* in){
                        if(!in) return;
                        in->setLSstride(LSstride);
                        in->setLSbounds(LSbounds);
                        in->setLSstart(0);
                    };
                    annotateInput(selOp0);
                    annotateInput(selOp1);
                    // Rewire: for each LOAD using this GEP, bypass address path and feed value SELECT
                    auto outNodes = node->outputNodes();
                    for(auto out : outNodes){
                        auto outIns = out->instruction();
                        if(outIns && outIns->getOpcode() == Instruction::Load){
                            // Connect SELECT's value to the consumers of the LOAD
                            auto loadUsers = out->outputNodes();
                            for(auto u : loadUsers){
                                int idx = u->getInputIdx(out);
                                auto e = edge(out, u);
                                EdgeType tp = e ? e->type() : EDGE_TYPE_DATA;
                                bool isBack = u->isInputBackEdge(out);
                                delEdge(edge(out, u));
                                connectNodes(selNode, u, idx, tp, isBack);
                            }
                            delNode(out);
                        }
                    }
                    // Finally, remove the GEP itself; we do not generate address ops
                    delNode(node);
                    continue; // proceed to next GEP
                }
            }
        }
        //delete alloca node
        ...
        // (old heuristic annotation for INPUTs removed; see SELECT→value rewrite earlier.)
        if(!newNodes.empty()){
        ...
        for(auto outputNode : node->outputNodes()){
            ...
            // TODO: how to do in multiTask
            if(ins == NULL && outputNode->customInstruction() == "OUTPUT"){
                errs() << outputNode->getName() << ": the output is outter node, continue\n";
                continue;
            }
            // For real Load/Store, set LS offset once
            if(ins && (dyn_cast<LoadInst>(ins) || dyn_cast<StoreInst>(ins))){
...
void LLVMCDFG::lowerFMulAddCalls()
{
    std::vector<LLVMCDFGNode*> fmaNodes;
    ...
    for(auto &elem : _nodes){
        ...
        Instruction *ins = node->instruction();
        if(!ins) continue;
        if(ins->getOpcode() != Instruction::Call) continue;
        if(auto CI = dyn_cast<CallInst>(ins)){
            Function* callee = CI->getCalledFunction();
            if(callee && callee->getName().startswith("llvm.fmuladd.")){
                fmaNodes.push_back(node);
            }
        }
    }
    for(auto node : fmaNodes){
        // Expect 3 operands: a, b, c
        auto a = node->getInputPort(0);
        auto b = node->getInputPort(1);
        auto c = node->getInputPort(2);
        BasicBlock* BB = node->BB();
        // create FMUL node
        LLVMCDFGNode* mulNode = addNode("FMUL", BB);
        mulNode->addInputNode(a, 0);
        a->addOutputNode(mulNode);
        addEdge(a, mulNode, EDGE_TYPE_DATA);
        mulNode->addInputNode(b, 1);
        b->addOutputNode(mulNode);
        addEdge(b, mulNode, EDGE_TYPE_DATA);
        // create FADD node
        LLVMCDFGNode* addNodeF = addNode("FADD", BB);
        addNodeF->addInputNode(mulNode, 0);
        mulNode->addOutputNode(addNodeF);
        addEdge(mulNode, addNodeF, EDGE_TYPE_DATA);
        addNodeF->addInputNode(c, 1);
        c->addOutputNode(addNodeF);
        addEdge(c, addNodeF, EDGE_TYPE_DATA);
        // redirect outputs of original node to addNodeF
        auto outNodes = node->outputNodes();
        for(auto out : outNodes){
            if(!out || out == node) continue;
            int idx = out->getInputIdx(node);
            if(idx < 0) continue;
            (void)out->delInputNode(node);
            delEdge(edge(node, out));
            out->addInputNode(addNodeF, idx);
            addNodeF->addOutputNode(out);
            addEdge(addNodeF, out, EDGE_TYPE_DATA);
        }
        // remove original fmuladd node
        delNode(node);
    }
}

// Detect ICMP + SELECT ladders that behave like min/max and replace them with Min/Max nodes
void LLVMCDFG::foldCmpSelectToMinMax()
{
    struct MinMaxPattern {
        LLVMCDFGNode* selectNode = nullptr;
        LLVMCDFGNode* condNode = nullptr;
        LLVMCDFGNode* trueNode = nullptr;
        LLVMCDFGNode* falseNode = nullptr;
        bool isMax = false;
        bool isFloat = false;
    };

    std::vector<MinMaxPattern> patterns;
    patterns.reserve(_nodes.size());

    auto nodesEquivalent = [](LLVMCDFGNode *lhs, LLVMCDFGNode *rhs) -> bool {
        if(lhs == rhs){
            return true;
        }
        if(!lhs || !rhs){
        ...
        // Constants are recreated per use; treat identical constants as equivalent.
        if(lhs->customInstruction() == "CONST" && rhs->customInstruction() == "CONST"){
            if(lhs->hasConst() && rhs->hasConst() &&
               lhs->constVal() == rhs->constVal()){
                int lhsBits = lhs->dataBits();
                int rhsBits = rhs->dataBits();
                if(lhsBits == 0 || rhsBits == 0 || lhsBits == rhsBits){
                    return true;
                }
            }
        }
    ...
    auto predicatePref = [](CmpInst::Predicate pred, bool &prefSmall, bool &prefLarge){
        switch(pred){
    ...
    for(auto &elem : _nodes){
        auto *selNode = elem.second;
        Instruction *selIns = selNode->instruction();
        if(!selIns) continue;
        if(selIns->getOpcode() != Instruction::Select) continue;
        if(selIns->getType()->isVectorTy()) continue;
        bool isIntSel = selIns->getType()->isIntegerTy() && selIns->getType()->getPrimitiveSizeInBits() > 1;
        bool isFloatSel = selIns->getType()->isFloatingPointTy();
        if(!isIntSel && !isFloatSel) continue;

        LLVMCDFGNode *condNode = selNode->getInputPort(2);
        if(!condNode) continue;
        Instruction *condIns = condNode->instruction();
        if(!condIns) continue;
        auto *icmp = dyn_cast<ICmpInst>(condIns);
        auto *fcmp = dyn_cast<FCmpInst>(condIns);
        if(!icmp && !fcmp) continue;

        LLVMCDFGNode *lhs = condNode->getInputPort(0);
        LLVMCDFGNode *rhs = condNode->getInputPort(1);
        LLVMCDFGNode *trueVal = selNode->getInputPort(0);
        LLVMCDFGNode *falseVal = selNode->getInputPort(1);
        if(!lhs || !rhs || !trueVal || !falseVal) continue;
        if(nodesEquivalent(trueVal, falseVal)) continue;

        bool trueIsLHS = nodesEquivalent(trueVal, lhs);
        bool trueIsRHS = nodesEquivalent(trueVal, rhs);
        bool falseIsLHS = nodesEquivalent(falseVal, lhs);
        bool falseIsRHS = nodesEquivalent(falseVal, rhs);
        if(!((trueIsLHS && falseIsRHS) || (trueIsRHS && falseIsLHS))){
            continue;
        }

        bool condPrefersSmall = false;
        bool condPrefersLarge = false;
        if(icmp){
            predicatePref(icmp->getPredicate(), condPrefersSmall, condPrefersLarge);
        }else{
            predicatePref(fcmp->getPredicate(), condPrefersSmall, condPrefersLarge);
        }
        if(!condPrefersSmall && !condPrefersLarge){
            continue;
        }
        ...
        patterns.push_back({selNode, condNode, trueVal, falseVal, isMax, isFloatSel});
    }

    for(auto &pattern : patterns){
        auto *selectNode = pattern.selectNode;
        if(!selectNode){
            continue;
        }
        ...
        std::string opcode;
        if(pattern.isFloat){
            opcode = pattern.isMax ? "FMAX" : "FMIN";
        }else{
            opcode = pattern.isMax ? "MAX" : "MIN";
        }
        LLVMCDFGNode *mmNode = addNode(opcode, selectNode->BB());

        auto wireInput = [&](LLVMCDFGNode *valueNode, int newIdx){
            if(!valueNode){
                return;
            }
            bool isBackEdge = selectNode->isInputBackEdge(valueNode);
            CondVal cond = selectNode->getInputCondVal(valueNode);
            EdgeType edgeType = EDGE_TYPE_DATA;
            if(auto existingEdge = edge(valueNode, selectNode)){
                edgeType = existingEdge->type();
            }
            valueNode->addOutputNode(mmNode, isBackEdge, cond);
            mmNode->addInputNode(valueNode, newIdx, isBackEdge, cond);
            addEdge(valueNode, mmNode, edgeType);
        };

        wireInput(pattern.trueNode, 0);
        wireInput(pattern.falseNode, 1);

        const auto &outputsRef = selectNode->outputNodes();
        std::vector<LLVMCDFGNode*> outputs(outputsRef.begin(), outputsRef.end());
        for(auto *out : outputs){
            if(!out){
                continue;
            }
            int idx = out->getInputIdx(selectNode);
            if(idx < 0){
                continue;
            }
            bool isBackEdge = out->isInputBackEdge(selectNode);
            CondVal cond = out->getInputCondVal(selectNode);
            LLVMCDFGEdge *oldEdge = edge(selectNode, out);
            EdgeType edgeType = EDGE_TYPE_DATA;
            if(oldEdge){
                edgeType = oldEdge->type();
            }
            out->delInputNode(selectNode);
            if(oldEdge){
                delEdge(oldEdge);
            }else{
                selectNode->delOutputNode(out);
            }
            out->addInputNode(mmNode, idx, isBackEdge, cond);
            mmNode->addOutputNode(out, isBackEdge, cond);
            addEdge(mmNode, out, edgeType);
        }

        delNode(selectNode);

        auto *condNode = pattern.condNode;
        if(condNode && condNode->outputNodes().empty()){
            auto condInputs = condNode->inputNodes();
            delNode(condNode);
            for(auto *in : condInputs){
                if(!in){
                    continue;
                }
                if(in->customInstruction() == "CONST" && in->outputNodes().empty()){
                    delNode(in);
                }
            }
        }
...
// Lower llvm.smax/smin/umax/umin calls into ICMP + SELECT nodes
void LLVMCDFG::lowerMinMaxCalls()
{
    std::vector<LLVMCDFGNode*> mmNodes;
    // collect first to avoid iterator invalidation
    for(auto &elem : _nodes){
        auto node = elem.second;
        Instruction *ins = node->instruction();
        ...
        bool isIntMinMax = name.startswith("llvm.smax.") || name.startswith("llvm.smin.") ||
                           name.startswith("llvm.umax.") || name.startswith("llvm.umin.");
        bool isFloatMinMax = name.startswith("llvm.maxnum.") || name.startswith("llvm.minnum.") ||
                             name.startswith("llvm.maximum.") || name.startswith("llvm.minimum.");
        if(isIntMinMax || isFloatMinMax){
            mmNodes.push_back(node);
        }
    }
    for(auto node : mmNodes){
        auto *CI = cast<CallInst>(node->instruction());
        Function* callee = CI->getCalledFunction();
        auto name = callee->getName();
        // Expect 2 operands: a, b
        auto a = node->getInputPort(0);
        auto b = node->getInputPort(1);
        BasicBlock* BB = node->BB();
        bool isFloat = name.startswith("llvm.maxnum.") || name.startswith("llvm.minnum.") ||
                       name.startswith("llvm.maximum.") || name.startswith("llvm.minimum.");
        bool isMax = name.contains("max");
        std::string opcode;
        if(isFloat){
            opcode = isMax ? "FMAX" : "FMIN";
        }else{
            opcode = isMax ? "MAX" : "MIN";
        }
        LLVMCDFGNode* MinMaxNode = addNode(opcode, BB);
        MinMaxNode->addInputNode(a, 0);
        a->addOutputNode(MinMaxNode);
        addEdge(a, MinMaxNode, EDGE_TYPE_DATA);
        MinMaxNode->addInputNode(b, 1);
        b->addOutputNode(MinMaxNode);
        addEdge(b, MinMaxNode, EDGE_TYPE_DATA);
        auto outNodes = node->outputNodes();
        for(auto out : outNodes){
            int idx = out->delInputNode(node);
            delEdge(edge(node, out));
            out->addInputNode(MinMaxNode, idx);
            MinMaxNode->addOutputNode(out);
            addEdge(MinMaxNode, out, EDGE_TYPE_DATA);
        }
...
{
    std::vector<LLVMCDFGNode*> bitcastNodes;
    for(auto &elem : _nodes){
        auto node = elem.second;
        Instruction *ins = node->instruction();
        std::string customIns = node->customInstruction();
        if(!customIns.empty()){
            ...
            }else if(customIns == "ACC" || customIns == "CACC" || customIns == "CIACC" || customIns == "CDIACC"){
                // Display FP accumulators with F* prefix in final opcode for DOT/debug
                bool isFP = false;
                auto in0 = node->getInputPort(0);
                if(in0) isFP = isFloatValueNode(in0);
                if(isFP){
                    if(customIns == "ACC") customIns = "FACC";
                    else if(customIns == "CACC") customIns = "FCACC";
                    else if(customIns == "CIACC") customIns = "FCIACC";
                    else if(customIns == "CDIACC") customIns = "FCDIACC";
                }
            }else if(customIns == "INPUT"){
                customIns = "Input";
            }else if(customIns == "OUTPUT"){
                customIns = "Output";
            }else if(customIns == "CONST"){
                customIns = "Const";
            }
            ...
            case Instruction::UDiv:
                node->setFinalInstruction("UDIV");
                break;
            case Instruction::SDiv:
                node->setFinalInstruction("SDIV");
                break;
            case Instruction::FDiv:
                // errs() << "Do not support Div Instructions\n";
                // assert(false);
                // break;
                node->setFinalInstruction("FDIV");
                break;
            case Instruction::URem:
            case Instruction::SRem:
            case Instruction::FRem:
                if(_suppRem){
                    node->setFinalInstruction("REM");
                    break;
                }else{
            ...
            case Instruction::FPToSI:{
                node->setFinalInstruction("FPTOSI");
                break;
            }
            case Instruction::SIToFP:{
                node->setFinalInstruction("SITOFP");
                break;
            }
            case Instruction::BitCast:{
                bitcastNodes.push_back(node);
                break;
            }
            ...
            case Instruction::FCmp:{
                // Map floating-point predicates onto existing compare ops.
                // We canonicalize GE/GT into LE/LT by swapping operands, as done for ICmp.
                // Note: We ignore ordered/unordered distinction here; kernels typically avoid NaNs.
                CmpInst *CI = dyn_cast<CmpInst>(ins);
                switch(CI->getPredicate()){
                case CmpInst::FCMP_OEQ:
                case CmpInst::FCMP_UEQ:
                    node->setFinalInstruction("FEQ");
                    break;
                case CmpInst::FCMP_ONE:
                case CmpInst::FCMP_UNE:
                    node->setFinalInstruction("FNE");
                    break;
                case CmpInst::FCMP_OGE:
                case CmpInst::FCMP_UGE:{
                    // a >= b  ->  b <= a
                    node->setFinalInstruction("FLE");
                    LLVMCDFGNode* op0 = node->getInputPort(0);
                    LLVMCDFGNode* op1 = node->getInputPort(1);
                    node->setInputIdx(op0, 1);
                    node->setInputIdx(op1, 0);
                    break;
                }
                ...
                case CmpInst::FCMP_ORD:
                    // Ordered: neither operand is NaN. Approximate as (a == a) && (b == b).
                    // Use EQ on same operand; downstream logic treats as boolean.
                    node->setFinalInstruction("FEQ");
                    break;
                case CmpInst::FCMP_UNO:
                    // Unordered: either operand is NaN. Approximate as NE on same operand.
                    node->setFinalInstruction("FNE");
                    break;
                case CmpInst::FCMP_TRUE:
                    // Always true; represent with EQ and rely on constant folding elsewhere if any.
                    node->setFinalInstruction("FEQ");
                    break;
                case CmpInst::FCMP_FALSE:
                    // Always false; represent with NE.
                    node->setFinalInstruction("FNE");
                    break;
                default:
                    assert(false);
            ...
            case Instruction::FNeg:{
                node->setInputIdx(node->inputNodes()[0], 1);
                LLVMCDFGNode* constZero = addNode("CONST", node->BB());
                constZero->setConstVal(0);
                connectNodes(constZero, node, 0, EDGE_TYPE_DATA);
                node->setFinalInstruction("FSUB");
                break;
            }
            ...
            case Instruction::Call:{
                CallInst* call_ins = dyn_cast<CallInst>(ins);
                assert(call_ins != nullptr);

                Function* calledFunc = call_ins->getCalledFunction();
                if (!calledFunc)  assert(false);

                //// Get this hardware implemented op's name from 
                //// the attributes of this function.
                //// We already add the "__CGRA_Hardware_Op" attribute
                //// in ParseHardwareImplementFunction().
                std::string funcOpName = "";
                if(calledFunc->hasFnAttribute("__CGRA_Hardware_Op")){
                    funcOpName = calledFunc->getFnAttribute("__CGRA_Hardware_Op").getValueAsString().str();
                } else{
                    assert(false && "Not a hardware-implemented op."); 
                }

                assert(funcOpName != "" && "Is not a hardware defined call op.");
                node->setFinalInstruction(funcOpName);
                break;
            }
            //////// End of jhlou add 20250418
    ...
    for(auto bitcastNode : bitcastNodes){
        bypassBitcastNode(bitcastNode);
    }
}
...
int castConstInt(Constant* V){
    ...
    {
        // For addr/stride math, rounding to int is intended
...
void LLVMCDFG::handleAffineLSNodes(){
    ...
    for(auto &elem : nodes){
        ...
        Instruction *ins = node->instruction();
        std::string customIns = node->customInstruction();
        if(node->isLSaffine() && ins != NULL && (dyn_cast<LoadInst>(ins) || dyn_cast<StoreInst>(ins))){
...
bool isADDNode(LLVMCDFGNode* node){
    ...
    auto ins = node->instruction();
    if(ins){
        if(ins->getOpcode() == Instruction::Add || ins->getOpcode() == Instruction::FAdd){
            return true;
        }
    }
    auto isAddMnemonic = [](const std::string& op)->bool{
        return op == "ADD" || op == "FADD";
    };
    if(isAddMnemonic(node->customInstruction())) return true;
    if(isAddMnemonic(node->finalInstruction())) return true;
    return false;
}

// Determine if an ADD-like node is floating-point (FAdd)
static bool isFloatADDNode(LLVMCDFGNode* node){
    if(auto ins = node->instruction()){
        return ins->getOpcode() == Instruction::FAdd;
    }
    if(node->customInstruction() == "FADD") return true;
    if(node->finalInstruction() == "FADD") return true;
    return false;
}

// Determine if a value-producing node is floating-point
static bool isFloatValueNode(LLVMCDFGNode* node){
    if(!node) return false;
    if(auto ins = node->instruction()){
        if(ins->getType()->isFloatingPointTy()) return true;
    }
    auto isFpOpStr = [](const std::string& s)->bool{
        return s == "FADD" || s == "FMUL" || s == "FSUB" || s == "FDIV" ||
               s == "FEQ"  || s == "FNE"  || s == "FLT"|| s == "FLE";
    };
    if(isFpOpStr(node->customInstruction())) return true;
    if(isFpOpStr(node->finalInstruction())) return true;
    return false;
}

// Helper to detect whether an edge is a loop back-edge even if only one side
// (input/output) recorded the metadata during previous transforms.
static bool hasBackEdgeBetween(LLVMCDFGNode* src, LLVMCDFGNode* dst){
    if(!src || !dst) return false;
    if(src->isOutputBackEdge(dst)) return true;
    if(dst->isInputBackEdge(src)){
        src->setOutputBackEdge(dst, true);
        return true;
    }
    return false;
}
        ...
        if(node->customInstruction() == "SELECT"){
            ...
            else
            {
                ...
                if (std::count(Addinputs.begin(), Addinputs.end(), op[1 - Addidx]))
                {
                    ...
                    bool useFADD = isFloatADDNode(op[Addidx]);
                    auto NewADD = addNode(useFADD ? "FADD" : "ADD", sameOperand->BB());
                    auto NewConst0 = addNode("CONST", sameOperand->BB());
                    NewConst0->setConstVal(0);
                    if(useFADD){
                        if(auto ai = op[Addidx]->instruction()){
                            NewConst0->setDataBits((int)ai->getType()->getPrimitiveSizeInBits());
                        }
                    }
            ...
            while (!SelsStack.empty())
            {
                ...
                for (auto outputNode : temOutputs)
                {
                    ...
                    if (outputNode->customInstruction() == "SELECT")
                    {
                        ...
                        if (anotherSelOp && anotherSelOp->customInstruction() == "SELECT" && selectTree.count(anotherSelOp))
                        {
            ...
            for (auto &treeNodeElem : selectTree)
            {
                ...
                {
                    bool useFADD = false;
                    if(isADDNode(op[0]) && isFloatADDNode(op[0])) useFADD = true;
                    if(isADDNode(op[1]) && isFloatADDNode(op[1])) useFADD = true;
                    auto NewADD = addNode(useFADD ? "FADD" : "ADD", treeNode->BB());
...
bool analyzeSelectTree(LLVMCDFGNode* node, LLVMCDFGNode* rootnode, std::set<LLVMCDFGNode*> &selectTree, bool &promisingCycle){
    ...
    for(auto inputNode: node->inputNodes()){
        ...
        if(inputNode == rootnode || selectTree.count(inputNode)){
            isReachable = true;
    ...
    if(isReachable){
        ...
        errs() << "\t{" << node->getName() << "} is reachable!\n";
...
void LLVMCDFG::generateACC(std::set<LLVMCDFGNode *> &InitialSels){
    ...
    for (auto node : temSet)
    {
        ...
        if (hasBackEdgeBetween(pre, node))
        {
            if (isADDNode(pre))
            {
                ...
                if (cycleout == NULL){
                    ...
                    {                                    // only output to addself
                        ...
                        {
                        if (hasBackEdgeBetween(pre, addsucc))
                ...
                // If this is a floating accumulation, tag zero width for DOT readability
                if(auto nins = node->instruction()){
                    if(nins->getType()->isFloatingPointTy()){
                        constantZero->setDataBits((int)nins->getType()->getPrimitiveSizeInBits());
                    }
                }
                delEdge(edge(Initial, node));
        ...
        if(isACC){
            ...
            if(Initial->hasConst()){
                ...
                node->setInitial(0);
                bool useFADD = isFloatADDNode(pre);
                auto InitialADD = addNode(useFADD ? "FADD" : "ADD", Initial->BB());
            ...
            bool useFADD = isFloatADDNode(pre);
            if(useFADD){
                node->setCustomInstruction("FACC");
            }else{
...
void LLVMCDFG::generateCACC(std::set<LLVMCDFGNode *> &InitialSels){
    ...
    for (auto node : InitialSels)//check if SELECT nodes left forming conditional select
    {
        ...
        if (!hasBackEdgeBetween(SelParent1, node))
        {
        ...
        if (findCACC)
        {
            ...
            { // add CTRLNOT after condition if opidx 1 correspond add result
                if(condition->customInstruction() == "CTRLNOT"){
                    // already inverted; nothing to do
                }
                else{   
            ...
            {
                node->setInitial(0);
                bool useFADD = isFloatADDNode(ADDNode);
                auto InitialADD = addNode(useFADD ? "FADD" : "ADD", Initial->BB());
                auto ACCOutputs = node->outputNodes();
                for (auto ACCOutput : ACCOutputs)
                {
            ...
            bool useFADD = isFloatADDNode(ADDNode);
            if(useFADD){
                node->setCustomInstruction("FCACC");
            }else{
...
void LLVMCDFG::handleSelectNodes(){
    ...
    for (auto &elem : _nodes)
    {
        ...
        if (node->customInstruction() == "SELECT")
        {
            errs() << "check SELECT: " << node->getName() << ": "; node->instruction()->dump();
            ...
            while (1)
            {
                ...
                for (auto initout : initSel->outputNodes())
                {
                    ...
                    }else if(initout->customInstruction() == "InitSELECT" && initout->getInputIdx(initSel) == 0){
                        if (pre->isOutputBackEdge(initout))
                        {
                            errs() << "\t\ttem init SELECT is "<< initout->getName() << "\n";
                            skipSel.insert(initout);
    ...
    for(auto rm : rmnodes){
        errs() << "remove node: "<<rm->getName()<< "\n";
        if(rm->customInstruction() == "InitSELECT"){
            InitialSels.erase(rm);
        }
        delNode(rm);
    }
    rmnodes.clear();
    ...
    for (auto node : InitialSels){
        //auto node = *&elem;
        errs() << "--------analyze root node {" << node->getName() << "}----\n";
...
void LLVMCDFG::dedupMemoryInputNodes(){
    std::vector<LLVMCDFGNode*> snapshot;
    snapshot.reserve(_nodes.size());
    for(auto &elem : _nodes){
        snapshot.push_back(elem.second);
    }

    std::unordered_map<std::string, LLVMCDFGNode*> signatureMap;
    for(auto *node : snapshot){
        if(!node) continue;
        if(!_nodes.count(node->id())) continue;

        std::string finalInstr = node->finalInstruction();
        Instruction *ins = node->instruction();
        if(!ins) continue;
        auto *LI = dyn_cast<LoadInst>(ins);
        if(!LI) continue;
        if(LI->isVolatile()) continue;
        if(!node->isLSaffine() && !_IOInfoMap.count(node)){
            continue;
        }

        std::string refName;
        if(_IOInfoMap.count(node)){
            refName = _IOInfoMap[node];
        }else if(node->isLSaffine()){
            refName = node->getLSArrayName();
        }

        std::ostringstream sigBuilder;
        sigBuilder << finalInstr << "|" << refName << "|" << node->dataBits();

        if(node->isLSaffine()){
            auto stride = node->getLSstride();
            sigBuilder << "|affine|"
                       << node->getLSoffset().to_string() << "|"
                       << node->getLSstart().to_string() << "|";
            std::vector<std::pair<int, varType>> strideVec(stride.begin(), stride.end());
            llvm::sort(strideVec, [](const auto &a, const auto &b){ return a.first < b.first; });
            for(auto &kv : strideVec){
                sigBuilder << kv.first << ":" << kv.second.to_string() << ";";
            }
        }else{
            sigBuilder << "|nonaffine";
        }

        std::string signature = sigBuilder.str();
        auto insertResult = signatureMap.try_emplace(signature, node);
        if(insertResult.second){
            continue;
        }

        LLVMCDFGNode *repNode = insertResult.first->second;
        if(repNode == node){
            continue;
        }
        std::vector<LLVMCDFGNode*> outputs = node->outputNodes();
        for(auto *outNode : outputs){
            if(!outNode) continue;
            int idx = outNode->getInputIdx(node);
            if(idx < 0){
                if(auto strayEdge = edge(node, outNode)){
                    delEdge(strayEdge);
                }else{
                    outNode->delInputNode(node);
                    node->delOutputNode(outNode);
            ...
            CondVal cond = outNode->getInputCondVal(node);
            EdgeType edgeType = EDGE_TYPE_DATA;
            if(auto bridgeEdge = edge(node, outNode)){
                edgeType = bridgeEdge->type();
                delEdge(bridgeEdge);
            }else{
                outNode->delInputNode(node);
            ...
            if(edge(repNode, outNode) == nullptr){
                repNode->addOutputNode(outNode, isBackEdge, cond);
                outNode->addInputNode(repNode, idx, isBackEdge, cond);
                auto *newEdge = addEdge(repNode, outNode, edgeType);
                if(newEdge){
                    newEdge->setType(edgeType);
                }
            }else{
                repNode->setOutputBackEdge(outNode, isBackEdge);
                repNode->setOutputCondVal(outNode, cond);
                outNode->setInputIdx(repNode, idx);
                outNode->setInputBackEdge(repNode, isBackEdge);
                outNode->setInputCondVal(repNode, cond);
                if(auto *existingEdge = edge(repNode, outNode)){
                    existingEdge->setType(edgeType);
                }
            }
        }

        if(_IOInfoMap.count(node)){
            if(!_IOInfoMap.count(repNode)){
                _IOInfoMap[repNode] = _IOInfoMap[node];
            }
            _IOInfoMap.erase(node);
        }
...
void LLVMCDFG::dedupHardwareCallNodes(){
    std::vector<LLVMCDFGNode*> snapshot;
    snapshot.reserve(_nodes.size());
    for(auto &elem : _nodes){
        snapshot.push_back(elem.second);
    ...
    for(auto *node : snapshot){
        ...
        Function *callee = CI->getCalledFunction();
        if(!callee) continue;
        if(!callee->hasFnAttribute("__CGRA_Hardware_Op")){
            continue;
        }

        std::string funcOpName = callee->getFnAttribute("__CGRA_Hardware_Op").getValueAsString().str();
        std::ostringstream sigBuilder;
        sigBuilder << funcOpName << "|" << CI->arg_size();
        bool missingInput = false;
        for(unsigned argIdx = 0; argIdx < CI->arg_size(); ++argIdx){
            LLVMCDFGNode *inputNode = node->getInputPort(argIdx);
            if(!inputNode){
                missingInput = true;
                break;
            }
            sigBuilder << "|" << reinterpret_cast<std::uintptr_t>(inputNode);
        }
        if(missingInput){
            continue;
        }
...
// Remove stores that are overwritten later in the same loop iteration.
// We detect WAW (OUTPUT_DEP) with zero distance from this store to a subsequent store
// and drop the earlier store node from the graph.
void LLVMCDFG::pruneOverwrittenStores(){
    std::vector<LLVMCDFGNode*> toErase;
    for(auto &elem : _nodes){
        LLVMCDFGNode* n = elem.second;
        Instruction* ins = n->instruction();
        if(!ins) continue;
        if(!dyn_cast<StoreInst>(ins)) continue;
        for(auto out : n->outputNodes()){
            if(!out) continue;
            Instruction* oins = out->instruction();
            if(!oins || !dyn_cast<StoreInst>(oins)) continue;
            auto memEdge = edge(n, out);
            if(!memEdge || memEdge->type() != EDGE_TYPE_MEM) continue;
            // Dependence payload is maintained on nodes
            DependInfo dep = out->getSrcDep(n);
            if(dep.type == OUTPUT_DEP && dep.isConstDist && dep.distance == 0){
                toErase.push_back(n);
                break;
            }
        }
...
void LLVMCDFG::enforceprintDOT(std::string fileName) {
	std::ofstream ofs;
	ofs.open(fileName.c_str());
    ofs << "Digraph G {\n";
    // nodes
...
void LLVMCDFG::printAffineDOT(std::string fileName) {
    ...
    for(auto &elem : _nodes){
        ...
        if(node->isAcc()){
            ...
            if(ins == NULL && node->customInstruction() == "INPUT"){
                // Prefer element-size * inner-loop-count for synthesized INPUTs
                varType elem = LSbounds[2];
                varType cnt = 1;
                if(!getLoopsAffineCounts().empty()) cnt = getLoopsAffineCounts(0);
                size = elem * cnt;
            }else if(maxsize.count(arrayname) && (dyn_cast<LoadInst>(ins) || dyn_cast<StoreInst>(ins))){
            ...
            ofs << ", pattern=\"";
            if(ins == NULL && node->customInstruction() == "INPUT"){
                // For INPUT nodes, prefer a simple element-stride pattern at innermost loop
                auto LSbounds = node->getLSbounds();
                varType elem = LSbounds[2];
                varType cnt = 1;
                if(!getLoopsAffineCounts().empty()) cnt = getLoopsAffineCounts(0);
                ofs << elem.to_string() << ", " << cnt.to_string();
            }else{
                auto stride = node->getLSstride();
            ...
            #ifdef MEM_PARTITION
            ofs << ", controlstep=" << node->getCtrlStep();
            if(!node->getMultiPort() == 0){
                auto BNPair = getMemPartition(arrayname);
                ofs << ", B=" << BNPair.first << ", N=" << BNPair.second;
            }
            #endif
...
void LLVMCDFG::printAsSubTask(std::ofstream &ofs, int kernel) {
    ...
    std::map<Value*, int> minoffset;
    std::map<Value*, int> maxsize;
    ...
    for(auto &elem : _nodes){
        ...
        if(ins != NULL){
            if(node->getLSoffset().index() == 0){
                ...
                if(dyn_cast<LoadInst>(ins)){
                    auto arrayname = node->getLSArrayName();
                    auto addrValue = node->getLSaddress();
                    if(!minoffset.count(addrValue)){
                        minoffset[addrValue] = LSoffset;
                    }else if(minoffset[addrValue] > LSoffset){
                        minoffset[addrValue] = LSoffset;
                    }
                }
            }
    ...
    if(!getminOffsetMap().empty()){
        minoffset = getminOffsetMap();
    }
    ...
    for(auto &elem : _nodes){
        ...
        if(ins != NULL){
            if(dyn_cast<LoadInst>(ins)){
                ...
                int sizeint = std::get<int>(size.value);
                int LSoffset = std::get<int>(node->getLSoffset().value);
                if(minoffset[addrValue] < LSoffset){
                    size += (LSoffset-minoffset[addrValue]);
                }
                if(!maxsize.count(addrValue)){
                    maxsize[addrValue] = sizeint;
                }else if(maxsize[addrValue] < sizeint){
                    maxsize[addrValue] = sizeint;
                }
            }
        }
    ...
    if(!getmaxSizeMap().empty()){
        maxsize = getmaxSizeMap();
    }
    ...
    for(auto &elem : _nodes){
        ...
        if(node->isAcc()){
            ...
            varType offset;
            varType reduceoffset;
            if(minoffset.count(addrValue) && (dyn_cast<LoadInst>(ins) || dyn_cast<StoreInst>(ins))){
                offset = minoffset[addrValue];
            }else{
                offset = node->getLSoffset();
            }
            size += (LSbounds[1] - LSbounds[0]);
            size += LSbounds[2];
            if(maxsize.count(addrValue) && (dyn_cast<LoadInst>(ins) || dyn_cast<StoreInst>(ins))){
                size_all = maxsize[addrValue];
            }else{
                size_all = size;
            }
            if(offset % BITWIDTH == 0){
            ...
            // errs()<<arrayname<<" reduceoffset: "<<reduceoffset.to_string()<<"\n";
            ofs << ", size=\""<<size.to_string()<<"\"";
            ofs << ", size_all=\""<<size_all.to_string()<<"\"";
            ofs << ", offset=\"" << offset.to_string() <<", ";
            //assert(node->getLSstart()>=0);
            ofs <<reduceoffset.to_string()<<"\"";
            ofs << ", pattern=\"";
        ...
        // }
            ...
            if(dyn_cast<LoadInst>(ins) || dyn_cast<StoreInst>(ins)){
                ...
                if(minoffset.count(addrValue)){
                    ...
                    if(node->getLSoffset().index() != 2){
                        int LSoffset = std::get<int>(node->getLSoffset().value);
                        if(LSoffset > minoffset[addrValue]){
                            offset = minoffset[addrValue];
                        }else{
                            offset = LSoffset;
                        }
...
void LLVMCDFG::generateCDFG()
{
    ...
    // initialized CDFG
    printDOT(std::to_string(printTimes) + "_" + _name + "_init.dot");
    ...
    connectCtrlDepBBs();
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_connectCtrlDepBBs.dot");
    ...
    insertCtrlNotNodes();
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_insertCtrlNotNodes.dot");
    ...
    createCtrlOrTree();
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_createCtrlOrTree.dot");

    outs() << ">>>>>> Transfer PHINode to SELECT nodes\n";
    handlePHINodes(); 
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_handlePHINodes.dot");
    // exit(0);
    ...
    handleGEPNodes();
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_handleGEPNodes.dot");

    if(!_noACC){
        transAddOpSelectTree();
        printDOT(std::to_string(printTimes) + "_" + _name + "_after_transAddOpSelectTree.dot");
    }

    outs() << ">>>>>> Handle select Node\n";
    handleSelectNodes(); 
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_handleSelectNodes.dot");
    ...
    if(!_noACC){
        ...
        //printDOT(std::to_string(printTimes) + "_" + _name + "_after_genMACNodes.dot");
    }
    ...
    if(!_noPattern && isloopsAffine()){
        ...
        handleAffineLSNodes(); 
        printDOT(std::to_string(printTimes) + "_" + _name + "_after_handleAffineLSNodes.dot");
    }

    if(!isloopsAffine()){
        outs() << ">>>>>> Add task level control\n";
        addTaskCTRL();
        //printDOT(std::to_string(printTimes) + "_" + _name + "_after_addTaskCTRL.dot");
    }
    ...
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_handleConversionNodes.dot");

    outs() << ">>>>>> fuse constant input node with the mul output node for acc node\n";
    fuseAccConstNode();
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_fuseAccConstNode.dot");

    outs() << ">>>>>> fuse zero-based subtraction into const mul/div\n";
    fuseZeroSubConstOps();

    outs() << ">>>>>> simplify the control logic\n";
    simplifyCtrlLogic();
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_simplifyCtrlLogic.dot");

    outs() << ">>>>>> Remove redundant nodes\n";
    removeRedundantNodes();
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_removeRedundantNodes.dot");

    outs() << ">>>>>> add edge between two nodes that have memory dependence\n";
    addMemDepEdges();
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_addMemDepEdges.dot");

    // Collapse sequential stores to the same address within an iteration
    pruneOverwrittenStores();
    ...
    if(!_noPattern && isloopsAffine()){
        ...
        auto runningTimeMs = std::chrono::duration_cast<std::chrono::milliseconds>(end-_start).count();
        std::cout << "IterDisttence Solving Time: " << runningTimeMs << " (ms)" << std::endl;
    }

    outs() << ">>>>>> traverse all conflict memory accesses and find out conflict partition\n";
    memPartition();

    dedupMemoryInputNodes();
    dedupHardwareCallNodes();

    outs() << ">>>>>> Fold select-based min/max patterns\n";
    foldCmpSelectToMinMax();

    outs() << ">>>>>> Assign final node name\n";
    // Composite calls already handled earlier (splitFmulAddCalls). Avoid double-lowering here.
    lowerMinMaxCalls();
    assignFinalNodeName();
    printDOT(std::to_string(printTimes) + "_" + _name + "_after_assignFinalNodeName.dot");
    printAffineDOT("affine.dot");
    //printAffineDOT("affine_"+std::to_string(printTimes) + "_" + _name + ".dot");
