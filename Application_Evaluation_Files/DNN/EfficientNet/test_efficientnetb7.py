...
def load_imagenet_val(val_dir, batch_size=64, workers=4, pin_memory=True):
    ...
    val_dataset = datasets.ImageFolder(
        val_dir,
        transforms.Compose([
            transforms.Resize(600),  # EfficientNet-B7使用600
            transforms.CenterCrop(600),  # EfficientNet-B7使用600
            transforms.ToTensor(),
            normalizer
        ])
    )
...
def main():
    ...
    # 加载验证集
    print('\n正在加载验证集...')
    val_loader = load_imagenet_val(val_dir, batch_size=32, workers=4)
    
    # 加载EfficientNet-B7预训练模型
    print('\n正在加载EfficientNet-B7预训练模型...')
