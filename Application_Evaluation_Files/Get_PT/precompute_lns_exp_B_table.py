...
def _worker_compute(args: Tuple[int, float, float]) -> Tuple[float, float, bool]:
    idx, lower, step = args
    y_val = lower + idx * step
    true_val = math.exp(y_val)

    bits_in = float_to_q16_16_bits(y_val)
    try:
        result = LNS_TOP_FUNC(
            conv_type=3,
            seg=6,
            x_in=WORKER_PARAMS["x_in"],
            y_in=bits_in,
            n=WORKER_PARAMS["n"],
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Precompute LNS exp lookup table with 16.16 quantization."
    )
    parser.add_argument(
        "--param-file",
        default=os.path.join(SCRIPT_DIR, "LNS", "parameter.json"),
        help="Path to the parameter.json containing exp1616 configuration.",
    )
    parser.add_argument(
        "--func-name",
        default="exp1616",
    ...
    parser.add_argument(
        "--output",
        default=os.path.join(SCRIPT_DIR, "LNS", "exp_1616_B_table.pt"),
        help="Destination path for the serialized lookup table.",
    )
    parser.add_argument("--lower", type=float, default=-20.0, help="Lower bound of the domain.")
    parser.add_argument("--upper", type=float, default=2.0, help="Upper bound of the domain.")
    parser.add_argument(
        "--frac-bits",
        type=int,
        default=16,
    ...
    if fallback_count > 0:
        print(f"Warning: {fallback_count} evaluations fell back to true exp.")
