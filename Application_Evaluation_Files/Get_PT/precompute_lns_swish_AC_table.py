...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default=os.path.join(SCRIPT_DIR, "LNS", "swish_1616_C_table.pt"),
    ...
    plt.legend()
    values_plot_path = os.path.join(SCRIPT_DIR, f"values_vs_true_{args.func_name}.png")
    ...
    # 绘制误差图
    plt.figure(figsize=(12, 6))
    plt.plot(inputs_tensor.numpy(), abs_errors.numpy(), linewidth=0.5, alpha=0.7)
    plt.xlabel('Input Value', fontsize=12)
    plt.ylabel('Absolute Error', fontsize=12)
    plt.title(f'Absolute Error of LNS Swish Approximation ({args.func_name})', fontsize=14)
    plt.grid(True, alpha=0.3)
