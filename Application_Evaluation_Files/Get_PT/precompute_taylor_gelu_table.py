...
from compute_tylor_exp import exp_q16, float_to_q16, q16_to_float

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

Q = 16
ONE = 1 << Q
# HALF_Q16 = int(round(0.5 * ONE))
# SQRT_2_OVER_PI_Q16 = int(round(math.sqrt(2.0 / math.pi) * ONE))
# COEFF_Q16 = int(round(0.044715 * ONE))

HALF_Q16 = int(0.5 * ONE)
SQRT_2_OVER_PI_Q16 = int(math.sqrt(2.0 / math.pi) * ONE)
COEFF_Q16 = int(0.044715 * ONE)
...
def _div_q16(a: int, b: int) -> int:
    if b == 0:
        raise ZeroDivisionError("Division by zero in Q16.16 arithmetic.")
    numerator = a << Q
    if numerator >= 0:
...
def _quantized_tanh_q16(x_q16: int) -> int:
    two_x = x_q16 << 1
    exp_pos = exp_q16(two_x)
    numerator = exp_pos - ONE
    denominator = exp_pos + ONE
    return _div_q16(numerator, denominator)


def gelu_q16(x_q16: int) -> int:
    x_sq = _mul_q16(x_q16, x_q16)
    x_cu = _mul_q16(x_sq, x_q16)
    poly_term = _mul_q16(COEFF_Q16, x_cu)
    inner = x_q16 + poly_term
    scaled_inner = _mul_q16(SQRT_2_OVER_PI_Q16, inner)
    tanh_val = _quantized_tanh_q16(scaled_inner)
    one_plus_tanh = ONE + tanh_val
    product = _mul_q16(x_q16, one_plus_tanh)
    return _mul_q16(HALF_Q16, product)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Precompute GELU lookup table using Taylor-based exp approximation with 16.16 quantization."
    )
    parser.add_argument("--lower", type=float, default=-4.0, help="Lower bound of the domain.")
    parser.add_argument("--upper", type=float, default=4.0, help="Upper bound of the domain.")
    parser.add_argument(
        "--frac-bits",
        type=int,
        default=16,
    ...
    parser.add_argument(
        "--output",
        default=os.path.join(SCRIPT_DIR, "LNS", "new_gelu_taylor_4_1616_table.pt"),
    ...
    for idx in tqdm(indices, desc="Computing Taylor GELU table"):
        x = args.lower + idx * step
        bits_in = float_to_q16_16_bits(x)
        x_q16 = bits_to_q16_16_int(bits_in)
        approx_q16 = gelu_q16(x_q16)
        approx_val = q16_to_float(approx_q16)
        true_val = 0.5 * x * (1.0 + math.tanh(math.sqrt(2.0 / math.pi) * (x + 0.044715 * x**3)))
