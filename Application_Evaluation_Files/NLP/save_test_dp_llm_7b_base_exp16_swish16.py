...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export DeepSeek LLM 7B Base with Swish16/Exp16 lookup activations.")
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def _replace_silu_with_lookup(model: LlamaForCausalLM, table_path: Path | str) -> None:
    for block in model.model.layers:
        act_device = block.mlp.down_proj.weight.device
        lookup = Swish16Lookup(table_path)
        if act_device.type != "meta":
