...
DEFAULT_OUTPUT_DIR = Path("/capsule/home/huangdaiwei/test_zgb/model/dp_llm_7b_base_exp16_swish16_taylor")
DEFAULT_SOURCE_REPO = "deepseek-ai/deepseek-llm-7b-base"

MODEL_MODULE_NAME = "modeling_dp_llm_swish16_exp16_taylor"
MODEL_CLASS_NAME = "Swish16Exp16TaylorDpLlmForCausalLM"

_Q16_SCALE = float(1 << 16)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export DeepSeek LLM 7B Base with Swish/Exp Taylor lookup activations.")
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Base model identifier or local path. Defaults to the cached DeepSeek LLM 7B Base snapshot.",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def _replace_silu_with_lookup(model: LlamaForCausalLM, swish_lookup: SwishTaylorLookup) -> None:
    for block in model.model.layers:
        act_device = block.mlp.down_proj.weight.device
        lookup = swish_lookup
        if act_device.type != "meta":
...
class {model_class}(LlamaForCausalLM):
