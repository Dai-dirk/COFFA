...
void ParseHardwareImplementFunction(Function &F){
	...
	if (global_annos)
	{
		...
		for (int i = 0; i < a->getNumOperands(); i++)
		{
			...
			{
				std::string anno = cast<ConstantDataArray>(cast<GlobalVariable>(e->getOperand(1)->getOperand(0))->getOperand(0))->getAsCString().str();
				// llvm::errs() << anno << "\n";
				...
				if (std::regex_match(anno, match, pattern)) {
					extracted = match[1]; 
					fn->addFnAttr("__CGRA_Hardware_Op", extracted); // <-- add function annotation here
					// fn->getAttributes().dump();
				}				
			}
		}
	}
