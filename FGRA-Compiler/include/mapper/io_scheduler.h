...
private:
    ...
    std::map<std::string, int> _partitionedStartBank; //@yuan: the start bank index for the partitioned serious memory banks, <mem name, index>
    std::map<std::string, std::set<int>> _allocatedMultiportBank; // <name, <Id, bank>>
...
public:
    ...
    void genCfgDataOnly(Mapping *mapping, std::ostream &os);
    // get the config data and dependence
    void genCfgData(Mapping *mapping, std::ostream &os, std::ostream &os_bits);
    ...
    void execute(Mapping *mapping, std::ostream &os_func, std::ostream &os_call, std::ostream &os_bits);
    void execute(Mapping *mapping, std::ostream &os_call); // @yuan: just for test
    //@yuan: set the start bank index for the partitioned memory banks
    // void setStartBank (std::string memName, int index);
    // //@yuan: return the start bank index of the partitioned memory banks
    // int StartBankIdx();
};
