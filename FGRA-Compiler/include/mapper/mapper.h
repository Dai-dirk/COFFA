...
protected:
    ...
    bool _CGOnly = false; //@yuan: for coarse-only
...
public:
    ...
    void initializeDfg(bool modify = false);
    ...
    bool preMapCheckPhase1(ADG* adg, DFG* dfg);
    // check if the DFG can be mapped to the ADG according to the resources
    bool preMapCheckPhase2(ADG* adg, DFG* dfg);
    // map the DFG to the ADG
    virtual bool mapper() = 0;
    // mapper with running time
    //@hw_yuan: make the directory of result as input parameter 
    bool mapperTimed(std::string resultDir = "");
    ...
    bool execute(bool dumpConfig = true, bool dumpMappedViz = true, std::string resultDir = "", bool CGOnly = false);
    
    // get max latency of the mapped DFG
    int getMaxLat();


    // detect the loops on the DFG
    void getLoops(DFG* dfg);

    // @yuan: get the II of the DFG
    int getII();

    //@yuan: we need to check whether the sum of N exceeds the architecture limitation, if so, update the solution
    // void partitionCheck(ADG* adg, DFG* dfg);

    //@hw_yuan: packing the DFG Nodes into a bigger node
    void opsPacking(DFG* newDfg, std::string resultDir);
    //@hw_yuan: check whether these two node can be packed, based on the supported operations of GPE
    bool isCompatible(DFGNode* curNode, DFGNode* canNode);
    //@hw_yuan: packing the DFG Nodes into a bigger node based on AA_Pack in VPR
    void opsAAPacking(DFG* newDfg, std::string resultDir);
