...
// IOB has two mode: IB and OB, mutually exclusive
struct ADGNodeAttr
{  
    std::map<int, DFGNode*> dfgNodes; // <FU-type, DFGNode*>
    std::vector<DfgEdgePassAttr> dfgEdgePass;
    std::map<int, std::map<int, bool>> inPortUsed;  // <bit-width, <port-idx, used>>
    std::map<int, std::map<int, bool>> outPortUsed; // <bit-width, <port-idx, used>>
};
...
private:
    ...
    // <fu-id, <bit-width, FUDelayAttr >>
    std::map<int, std::map<int, FUDelayAttr>> _fuDelayAttr;

    // DFG edges with latency violation
    std::vector<int> _vioDfgEdges;
    std::map<std::string, std::set<int>> _multportIobs; //<multiport-name, occupied-IOBs>//@mult_port
    ...
    //bool routeDfgEdgeFromLUT(DFGEdge* edge, ADGNode* adgNode, DFGNode* unmapdfgNode);
    // @yuan:find a routable path from ALU to LUT
    //bool routeDfgEdgeFromALU(DFGEdge* edge, ADGNode* adgNode, DFGNode* unmapdfgNode);
    // @yuan: route inside the GPE
    bool routeDfgEdgeInsideGPE(DFGEdge* edge, ADGNode* targetNode);
    // find the available input ports in the dstNode to route edge
    std::set<int> availDstPorts(DFGEdge* edge, ADGNode* dstNode); 
public:
    Mapping(ADG* adg, DFG* dfg): _adg(adg), _dfg(dfg) {_II = dfg->MII();}
    ~Mapping(){}
    // void setDFG(DFG* dfg){ _dfg = dfg; }
    DFG* getDFG(){ return _dfg; }
    // void setADG(ADG* adg){ _adg = adg; }
    ADG* getADG(){ return _adg; }
    std::map<std::string, std::set<int>>& multportIobs(){return _multportIobs;}
    ...
    void reset();
    // return the number of mapped nodes in current iteration
    int numNodeMapped(){ return _numNodeMapped; }
    // if this input port of this ADG node is used
    bool isAdgNodeInPortUsed(int nodeId, int bits, int portIdx);
    // if this output port of this ADG node is used
    bool isAdgNodeOutPortUsed(int nodeId, int bits, int portIdx);
    // if the DFG node is already mapped
    bool isMapped(DFGNode* dfgNode);
    // if the FU in ADG node is already mapped, type: ALU, LUT, IOC
    bool isMapped(ADGNode* adgNode, int type);
    ...
    // if the DFG input node is mapped
    // bool isDfgInputMapped(int id);
    // // if the DFG output node is mapped
    // bool isDfgOutputMapped(int id);
    // occupied ADG node of this DFG node
    ADGNode* mappedNode(DFGNode* dfgNode);
    // mapped DFG node of this ADG node
    DFGNode* mappedNode(ADGNode* adgNode, int type);
    // mapped DFG nodes of this ADG node
    std::vector<DFGNode*> mappedNode(ADGNode* adgNode);
    // get mapped FU type of the DFG Node
    int mappedFUType(DFGNode* dfgNode);
    ...
    bool routeDfgEdge(DFGEdge* edge, ADGNode* adgNode);
    ...
    int getAvailDelay(FUNode* fuNode, DFGNode* dfgNode, int bitwidth);
    ...
    void insertPassDfgNodes(DFG* newDfg, bool CGOnly = false);


    int II(){ return _II; }
    void setII(int II) { _II = II; }
    // evaluate II according to loop latency
    int evaluateII();
    //@yuan: get the II
    int getII();
    // @yuan: clear all the historical usage of the ADGNodes
    void clearADGNodeUsage();
    int getADGNodeHisUsage(int ADGNodeId, int DFGNodeId);
    // @yuan: check current time slot is satisfied the memory denepdency
    bool isTimeAvai(DFGNode* node, int time);
    // @yuan: check whether current mapping result violates memory dependency
    int hasVioMemDenpendency(int vio); 
    // @yuan: check current time slot doesn't affect correctness
    int TaskLat(DFGNode* node, int time);
    //@yuan: delete edges with long iteration-distance
