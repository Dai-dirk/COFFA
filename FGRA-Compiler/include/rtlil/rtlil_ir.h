...
private:
    ...
    bool _isInput = false;//@hw_yuan_op: it is used to indicate the input edge of the sub-DFG, which needs to connect to the original DFG
    bool _isOutput = false;//@hw_yuan_op: it is used to indicate the input edge of the sub-DFG, which needs to connect to the original DFG
    int _inputId;//@hw_yuan_op
...
public:
    ...
    //@hw_yuan_op
    void setIsInput(bool isinput){ _isInput = isinput;}
    bool isInput(){ return _isInput; }
    void setIsOutput(bool isoutput){ _isOutput = isoutput;}
    bool isOutput(){ return _isOutput; }
    void setInputId(int id){ _inputId = id; }
    int inputId(){ return _inputId;}
};
