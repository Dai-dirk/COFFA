...
DFG* DFGIR::parseDFGJson(std::string filename){
    ...
    for(auto& nodeJson : dfgJson["objects"]){
        ...
        if(opName == "CONST"){
            if(nodeJson.contains("value")){
                std::string tmp_str = nodeJson["value"].get<std::string>();
                if(tmp_str.find('.') == std::string::npos){
                    value = std::stoull(tmp_str);
                }else{
                    float fpvalue = std::stof(tmp_str);
                    uint32_t bits32 = *reinterpret_cast<uint32_t*>(&fpvalue);
                    value = static_cast<int64_t>(bits32);
                }
            ...
            || opName == "TLOAD" || opName == "TSTORE" || opName == "TCLOAD" || opName == "TCSTORE"){
                ...
                if(nodeJson.contains("acc_params")){
                    ...
                    if(nodeJson.contains("second_initVal")){
                        ...
                        dfg_node->set2ndImm(initVal);
                        dfg_node->set2ndImmIdx(1); // @yuan: for the second initial value, its operand idx is fixed to 1
                    }
                ...
                //@yuan_argIn:
                if(opName == "ARGIN"){
                    dfg_node->setIsArgIn(true);
                    int dfgArgNum = dfg->numArgIn() + 1;
                    dfg->setNumArgIn(dfgArgNum);
                    int argIdx = std::stoi(nodeJson["argNum"].get<std::string>());
                    dfg_node->setArgIdx(argIdx);
                    cell->setParameters("argNum", nodeJson["argNum"].get<std::string>());
                }
    ...
    for(auto& edgeJson : dfgJson["edges"]){
        ...
        if(edgeJson.contains("Width")){
            ...
            if(dfg->isIONode(srcId) && dfg->isIONode(dstId)){
                bitWidth = 1;
            }
        }
        ...
        if(isConst(srcId)){ // merge const node into the node connected to it
            ...
            if(node->hasImm(bitWidth)){//@yuan_argIn: for the node has two constant
                node->set2ndImm(constValue(srcId));
                node->set2ndImmIdx(dstPort);
            }else{
