...
void addCfgData(std::map<int, CfgData> &cfg, const CfgDataLoc &loc, uint64_t data, int argInIdx){//@yuan_argIn: set the flag that this configuration is transmitted from outside
    int len = loc.high - loc.low + 1;
    CfgData loc_data(len);
    uint64_t val = data & (((uint64_t)1 << len) - 1);
    // std::cout << "len: " << len << " data: " << data << " val: " << val << " argInIdx: "<< argInIdx << std::endl;
    while(len > 0){
    ...
    loc_data.argInIdx = argInIdx;
    // std::cout << "loc.low: "<< loc.low << " loc_data.argInIdx: " << loc_data.argInIdx << std::endl;
    cfg[loc.low] = loc_data;
}
...
std::map<int, CfgData> Configuration::getGpeCfgData(GPENode* node){
    ...
    bool isMIMD = node->isMIMD();
    // std::cout << "is MIMD: " << isMIMD << std::endl;
    ...
    for(auto& mappedNode : adgNodeAttr.dfgNodes){
        ...
        // std::cout << "generate cfg for : " << dfgNode->name() << " in GPE: " << node->name() << std::endl;
        ...
        if( type == TYPE_ALU && dfgNode->operation() == "XCORE"){//@yuan_xcore: XCore has different architecture, so that it requires different configuration
            std::map<std::string, int> unifiedCfgMap = cfgMap();
            // std::cout <<  dfgNode->operation() << " opc: " << opc << " unified cfg: " << unifiedCfgMap[dfgNode->operation()] << std::endl;
            int lnsId = -1;
            std::map<int, int> usedRdu; // <width, RDU-id>
            for(auto& elem : dfgNode->inputEdges()){
                int bitWidth = elem.first;// for each bitwidth
                // std::cout << "input edge width: " << bitWidth << " size: " << elem.second.size() << std::endl;
                for(auto& edge : elem.second){ // for each edge
                    ...
                    if(!(dfgNode->accumulative() && bitWidth == 1)){
                        auto lnsPair = rdu->output(bitWidth, rduPort).begin();  // RDU has the same input/output index
                        lnsId = lnsPair->first;
                    }
            ...
            if(lnsId == -1){// in case that some node has no input
                ...
                // auto rdu = subAdg->node(cg_rduId);   
                // auto lnsPair = rdu->output(cg_width, 0).begin();   
                // lnsId = lnsPair->first;  
            }
            // for(auto& elem : node->cfgIdMap){
            //     std::cout << "cfgMap name: " << elem.first << " id: " << elem.second << std::endl;
            // }
            //@yuan_xcore: begin to config the n, logc, et.al
            auto xcoreAttr = dfgNode->getXcoreAttr();
            // n: for fixed point
            int nId = node->cfgIdMap["n"];
            addCfgData(cfg, node->configInfo(nId), xcoreAttr.n);
            // float_flag
            int floatId = node->cfgIdMap["fp_flag"];
            addCfgData(cfg, node->configInfo(floatId), (uint32_t)xcoreAttr.float_flag);
            // bias_sel
            int biasSelId = node->cfgIdMap["bias_in"];
            addCfgData(cfg, node->configInfo(biasSelId), (uint32_t)xcoreAttr.bias_sel);
            std::cout << "biasSelId: " << biasSelId << " config: " << xcoreAttr.bias_sel << std::endl;
            // opc
            int opcId = node->cfgIdMap["opc"];
            addCfgData(cfg, node->configInfo(opcId), (uint32_t)xcoreAttr.opc);
            // break point
            auto& breakPointsMap = xcoreAttr.break_points;
            for(auto& elem : breakPointsMap){
                std::string breakIdx = "break_points_" + std::to_string(elem.first);
                int breakId = node->cfgIdMap[breakIdx];
                addCfgData(cfg, node->configInfo(breakId), elem.second);
            }
            // constant_bias
            auto& biasMap = xcoreAttr.constant_bias;
            for(auto& elem : biasMap){// there are 6 constan bias for 6 segments
                std::string biasIdx = "constant_bias_" + std::to_string(elem.first);
                int biasId = node->cfgIdMap[biasIdx];
                addCfgData(cfg, node->configInfo(biasId), elem.second);
            }
            // K
            auto& kMap = xcoreAttr.k_in;
            for(auto& elem : kMap){
                std::string ksIdx = "K_in_" + std::to_string(elem.first);
                int kId = node->cfgIdMap[ksIdx];
                addCfgData(cfg, node->configInfo(kId), elem.second);
            }
            // LOGC  
            auto& logcMap = xcoreAttr.logc;  
            for(auto& seg : logcMap){
                for(auto& elem : seg.second){
                    std::string logcIdx = "logc_" + std::to_string(seg.first) + "_" + std::to_string(elem.first);
                    int logcId = node->cfgIdMap[logcIdx];
                    addCfgData(cfg, node->configInfo(logcId), elem.second);
                }
            }   
            ...
            for(auto& elem : dfgNode->bitWidths()){
                if(dfgNode->hasImm(elem)){
                    ...
                    // std::cout << " width: "<< elem <<" num_input: "<< dfgNode->numInputs(elem) << " index: " << i<< std::endl
                    int rduId = usedRdu[elem];
                    // std::cout << "coarse grained i: " << i << " rduId: " << rduId  << " usedRdu[elem] size: "<< usedRdu.size()<< std::endl;
                    // for(auto t : usedRdu){
                    //     std::cout << "rdu width: " << t.first << " id: " << t.second<<std::endl;
                    // }
                    ...
                    for(auto& elem_i : subAdg->node(muxId)->inputs(elem)){
                        ...
                        if(subAdg->node(id)->type() == "Const"){
                            ...
                            if(dfgNode->isConstArgIn())
                                argInIdx = dfgNode->getArgIdx();
                            addCfgData(cfg, node->configInfo(id), (uint64_t)(dfgNode->imm(elem).second), argInIdx);
                            break;
                        }
                    }  
                }
            }
        }else if( type == TYPE_ALU && dfgNode->operation() != "XCORE"){
            std::map<std::string, int> unifiedCfgMap = cfgMap();
            // std::cout <<  dfgNode->operation() << " opc: " << opc << " unified cfg: " << unifiedCfgMap[dfgNode->operation()] << std::endl;
            // std::cout << "ALU node operation: " << dfgNode->operation() << std::endl;
            int aluId = -1;
            std::map<int, int> usedRdu; // <width, RDU-id>
            // for(auto& elem : dfgNode->bitWidths()){
            //     if(elem != 1){
            ...
            for(auto& elem : dfgNode->inputEdges()){
                ...
                for(auto& edge : elem.second){ // for each edge
                    ...
                    DFGNode* src = _mapping->getDFG()->node(e->srcId()); 
                    DFGNode* dst = _mapping->getDFG()->node(e->dstId()); 
                    int addDelay = e->addDelay();//@hw_yuan: additional delay caused by packing
                    auto& edgeAttr = _mapping->dfgEdgeAttr(eid);
                    int delay = edgeAttr.delay; // delay cycles
                    int inputIdx = edgeAttr.edgeLinks.rbegin()->srcPort; // last edgeLInk, dst port
                    if(inputIdx == -1){// using the internal link in GPE
                        ...
                        if(dfgNode->accumulative() || dfgNode->operation() == "CISEL"){
                            ...
                            if(!dfgNode->isPacked() && node->isMIMD()){
                                Operand += 2;
                            }
                            inputIdx = *node->operandInputs(bitWidth, Operand).begin();
                        }
                        ...
                        usedRdu[bitWidth] = cg_rduId;
                        delayUsed[bitWidth][cg_rduId][rduPort] = edgeAttr.delay + addDelay; // delay cycles used by this port   
                        ...
                        // std::cout << "edge from: " << src->name() << " to: " << dst->name() << " width: " <<  bitWidth <<" inputIdx: "<<inputIdx << " addDelay: " << addDelay<< std::endl; 
                        continue;
                    }
                    auto muxPair = subAdg->input(bitWidth, inputIdx).begin(); // one input only connected to one Mux, @yuan: for sub-adg, the input() returns the relationship between inport and the sub-module
                    int muxId = muxPair->first;
                    ...
                    // std::cout << "edge from: " << src->name() << " to: " << dst->name() << " width: " <<  bitWidth <<" inputIdx: "<<inputIdx << " cg_rduId: " << cg_rduId << " rduPort: "<< rduPort << " edgeAttr.delay: "<< edgeAttr.delay <<" addDelay: " << addDelay << std::endl; 
            ...
            // std::cout << "============== "  << aluId << std::endl;
            if(isMIMD){//@hw_yuan: for MIMD-CGRA, there is no ALU explicitly
                if(dfgNode->isPacked()){
                    for(auto op : dfgNode->multiOps()){
                        std::string euName = _mapping->getDFG()->getEuType(op);
                        int euId = node->cfgIdMap[euName];
                        int cfgValue = 0;
                        auto cfgInfo = node->euConfigInfo(euName);
                        //first, we generate the operator_core's cfg
                        if(cfgInfo.CoreCfgWIdth != 0){
                            cfgValue = (cfgValue << cfgInfo.CoreCfgWIdth) | unifiedCfgMap[op];
                            // std::cout << "op: " << op << " cfg: " << unifiedCfgMap[op] << std::endl;
                        }
                        for(auto& interAttr : dfgNode->internalAttrMap(euName)){
                            if(interAttr.second.width == 1){//hw_yuan: for now, only consider 1 fine-grained operand 
                                if(interAttr.second.srcName == "GPE"){// there is no fine-grained const input
                                    int srcPort = interAttr.second.srcPort - node->numOperands(cg_width);
                                    int imuxCGWidth = cfgInfo.imuxCGCfgWIdth;
                                    cfgValue = (srcPort << (cfgInfo.CoreCfgWIdth + imuxCGWidth)) | cfgValue;
                                    // std::cout << "GPE to " << " Eu: " << euName << " srcPort: " << srcPort << std::endl;
                                }else{
                                    std::string srcEuName = interAttr.second.srcName;
                                    int srcEuOrder = node->FGEusId(srcEuName);
                                    int euOrder = node->FGEusId(euName);
                                    int operand = interAttr.first;
                                    int imuxCGWidth = cfgInfo.imuxCGCfgWIdth;
                                    //@hw_yuan: an interesting point is that the node generates fine-grained output doesn't need it
                                    // int srcPort = 0;
                                    // if(euOrder > srcEuOrder){
                                    //     srcPort = srcEuOrder + 1;
                                    // }else{
                                    //     srcPort = srcEuOrder;
                                    // }
                                    cfgValue = ((srcEuOrder + 1) << (cfgInfo.CoreCfgWIdth + imuxCGWidth)) | cfgValue;
                                    // std::cout << "srcEU: " << srcEuName << " order: " << srcEuOrder << " Eu: " << euName << " order: " << euOrder<< " operand: " << operand<< std::endl;
                                }
                            }else{
                                if(interAttr.second.srcName == "GPE" || interAttr.second.srcName == "CONST"|| interAttr.second.srcName == "Second_Const"){
                                    int srcPort = interAttr.second.srcPort;
                                    int operand = interAttr.first;
                                    int imuxCGWidth = cfgInfo.imuxCGCfgWIdth;
                                    if(srcPort > 0) srcPort -= 1;
                                    cfgValue = (srcPort << (cfgInfo.CoreCfgWIdth + operand * imuxCGWidth / 2)) | cfgValue;
                                }else{
                                    std::string srcEuName = interAttr.second.srcName;
                                    int srcEuOrder = node->CGEusId(srcEuName);
                                    int euOrder = node->CGEusId(euName);
                                    int operand = interAttr.first;
                                    int imuxCGWidth = cfgInfo.imuxCGCfgWIdth;
                                    int srcPort = 0;
                                    if(euOrder > srcEuOrder){
                                        srcPort = srcEuOrder + 3;
                                    }else{
                                        srcPort = srcEuOrder + 3 - 1;
                                    }
                                    // std::cout << "srcEU: " << srcEuName << " order: " << srcEuOrder << " Eu: " << euName << " order: " << euOrder<< " srcPort: " << srcPort << std::endl;
                                    cfgValue = (srcPort << (cfgInfo.CoreCfgWIdth + operand * imuxCGWidth / 2)) | cfgValue;
                                }
                            }
                        }
                        addCfgData(cfg, node->configInfo(euId), (uint32_t)cfgValue);
                        // std::cout << "one EU name: " << euName << " operation: " << op << " cfgValue: " << cfgValue << std::endl; 
                    }
                    std::string latestEuName = _mapping->getDFG()->getEuType(dfgNode->operation());
                    int outCGMuxId = node->cfgIdMap["outMuxCG"];
                    int outFGMuxId = node->cfgIdMap["outMuxFG"];
                    int euCGOrderIdx = node->CGEusId(latestEuName);
                    int euFGOrderIdx = node->FGEusId(latestEuName);
                    addCfgData(cfg, node->configInfo(outCGMuxId), (uint32_t)euCGOrderIdx);
                    addCfgData(cfg, node->configInfo(outFGMuxId), (uint32_t)euFGOrderIdx);
                    // std::cout << "EU name: " << latestEuName << " operation: " << dfgNode->operation() << " euCGOrderIdx: " << euCGOrderIdx << " euFGOrderIdx: " << euFGOrderIdx << std::endl; 
                }else{
                    std::string euName = _mapping->getDFG()->getEuType(dfgNode->operation());
                    int euId = node->cfgIdMap[euName];
                    int outCGMuxId = node->cfgIdMap["outMuxCG"];
                    int outFGMuxId = node->cfgIdMap["outMuxFG"];
                    auto cfgInfo = node->euConfigInfo(euName);
                    ...
                    //first, we generate the operator_core's cfg
                    if(cfgInfo.CoreCfgWIdth != 0){
                        cfgValue = (cfgValue << cfgInfo.CoreCfgWIdth) | unifiedCfgMap[dfgNode->operation()];
                    }
                    //second, we configure the input muxes
                    //for unpacked node, all the eu's inputs come from GPE
                    addCfgData(cfg, node->configInfo(euId), (uint32_t)cfgValue);
                    //finally, we configure the ALU's output muxes
                    int euCGOrderIdx = node->CGEusId(euName);
                    int euFGOrderIdx = node->FGEusId(euName);
                    addCfgData(cfg, node->configInfo(outCGMuxId), (uint32_t)euCGOrderIdx);
                    addCfgData(cfg, node->configInfo(outFGMuxId), (uint32_t)euFGOrderIdx);
                    // std::cout << "EU name: " << euName << " operation: " << dfgNode->operation() << " cfgValue: " << cfgValue << " euCGOrderIdx: " << euCGOrderIdx << " euFGOrderIdx: " << euFGOrderIdx << std::endl; 
                }
            }else{
                int opc = Operations::OPC(dfgNode->operation());//@hw_yuan: unified GPE doesn't need this opc
                // std::cout << "Normal ALU: " << dfgNode->operation() << " opc: " << opc << std::endl;
                //@yuan_fp: for the ALU supports FDIV/FSQURT, opc = {opc to select mux; ii for FDIV; conifg for unified unit}
                bool hasNoUnified = true;
                bool hasNoFdiv = true;
                for(auto& elem : node->operations()){
                    if(unifiedCfgMap.count(elem)){
                        opc <<= 3;
                        break;
                    }
                }
                //@yuan_fp: for accumulated-operations, we need to get its real operation to config the unified unit
                std::string RealOp = dfgNode->operation();
                if(dfgNode->accumulative()){
                    RealOp = realOp(dfgNode->operation());
                }
                if(unifiedCfgMap.count(RealOp)){
                    opc += unifiedCfgMap[dfgNode->operation()];
                }
            ...
            for(auto& elem : dfgNode->bitWidths()){
                if(dfgNode->hasImm(elem)){
                    if(elem == 1){ 
                        ...
                        int i = 0;
                        if(dfgNode->isPacked()){
                            i = dfgNode->imm(elem).first;
                            assert(!usedOperands[type].count(i));
                        }else{
                            for(; i < dfgNode->numInputs(elem); i++){
                                if(!usedOperands[type].count(i)){ 
                                    break;
            ...
            if(dfgNode->has2ndImm()){
                ...
                //     }
                // }
                int opIdx = dfgNode->get2ndImmIdx();
                // std::cout << "opIdx: " << opIdx << std::endl;
                assert(!usedOperands[type].count(opIdx));
                ...
                for(auto& elem_i : subAdg->node(muxId)->inputs(cg_width)){
                    ...
                    if(subAdg->node(id)->type() == "Second_Const"){
                        addCfgData(cfg, node->configInfo(muxId), (uint32_t)elem_i.first);
                        // std::cout << " id: " << id << std::endl;
                        // std::cout << " const: " << (int)dfgNode->get2ndImm() << std::endl;
                        addCfgData(cfg, node->configInfo(id), (uint64_t)(dfgNode->get2ndImm()));
                        break;
                    }
                }
            }
            ...
            if(dfgNode->accumulative()){
                ...
                // write interval
                int II =  _mapping->II();//ISEL_PARAM:[1]
                int WI = dfgNode->interval() * II;
                int wiId = node->cfgIdMap["WI"];
                addCfgData(cfg, node->configInfo(wiId), (uint32_t)WI);
                ...
                // write cycles
                int cycles = dfgNode->cycles();//ISEL_PARAM:[0]
                int cyclesId = node->cfgIdMap["Cycles"];
                addCfgData(cfg, node->configInfo(cyclesId), (uint32_t)cycles);
                // std::cout << "isel cycles: " << cycles << std::endl;
                //wirte repeats number
                int repeats = dfgNode->repeats();//ISEL_PARAM:[2]
                int repeatsId = node->cfgIdMap["Repeats"];
                addCfgData(cfg, node->configInfo(repeatsId), (uint32_t)repeats);
                // std::cout << "isel cycles: " << cycles << std::endl;
                // //if skip first accumulative num
    ...
    for(auto& eachwidth : delayUsed){
        ...
        for(auto& elem : eachwidth.second ){
            ...
            for(auto& used : elem.second){
                ...
                // std::cout << "width: " << bitWidth << " rduId: " << rduId << " port: " << used.first <<" delay: " << used.second << std::endl;
    ...
    // for(auto& elem: cfg){
    //     std::cout << "cfg low: " << elem.first << " arginidx: " << elem.second.argInIdx << std::endl;
    // }
    return cfg;
}
...
std::map<int, CfgData> Configuration::getIobCfgData(IOBNode* node){
    ...
    //TODO: here may be some bugs when the precision < data_width
...
void Configuration::getNodeCfgData(ADGNode* node, std::vector<CfgDataPacket>& cfg){
    ...
    // std::cout << "cfgmap size: " << cfgMap.size() <<" cfg totalLen: " << totalLen << " num: " << num << std::endl;
    std::vector<uint32_t> cfgDataVec(num, 0);
    std::set<uint32_t> addrs;
    std::map<uint32_t, int> addrArgIdxMap;//@yuan_argIn: this map is used to recodrd whether the cofiguration is a argument for one addr(the key)
    for(auto& elem : cfgMap){ // std::map auto-sort keys
        int lsb = elem.first;
        int len = elem.second.len;
        auto& data = elem.second.data;
        int agrInIdx = elem.second.argInIdx;
        // cache valid address
        uint32_t targetAddr = lsb/cfgDataWidth;
        // std::cout << "targetAddr: " << targetAddr << std::endl;
        int addrNum = (len + (lsb%cfgDataWidth) + cfgDataWidth - 1)/cfgDataWidth;
        for(int i = 0; i < addrNum; i++){
            addrs.emplace(targetAddr+i);
            addrArgIdxMap[targetAddr+i] = agrInIdx;
        } 
    ...
    for(auto addr : addrs){
        ...
        int argInIdx = addrArgIdxMap[addr];
        // std::cout << "real addr: " << std::hex << (highAddr|addr) << std::dec << " argInIdx: " << argInIdx << std::endl;
        if(cfgDataWidth >= 32){
...
//@yuan_hw: get the real op
std::string Configuration::realOp(std::string op){
    //TODO: add more operations support
    if(op == "ACC" || op == "CACC" || op == "CIACC" || op == "CDIACC"){
        return "ADD";
    }else if(op == "MAC" || op == "CMAC"){
        return "MUL";
    }
    return op;
}
