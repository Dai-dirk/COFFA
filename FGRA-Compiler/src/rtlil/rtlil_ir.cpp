        ...
        case 3:{ // current line start with "parameter"
            ...
            if(cell->operation() == "LUT"){// lut should be processed specially
                ...
                if(parName == "VALUE"){
                    ...
                    int64_t value = 0;
                    if(pos != -1){
                        value_s = pline[2].substr(pos + 1, pline[2].length() - 2);
                        value = std::stoll(value_s, 0, 10);
                    }else{
                        value = std::atoll(value_s.c_str());

                    }
                    cell->setValue(value);
                    // std::cout << "value_s: " << value_s << " pline[2]: " << pline[2] << " value:" << value <<std::endl;
        ...
        if(cell.second->operation() == "CONST"){
            ...
            || nodeOp == "TLOAD" || nodeOp == "TSTORE" || nodeOp == "TCLOAD" || nodeOp == "TCSTORE"){
                ...
                for(auto& par : cell.second->Parameters()){
                    ...
                    }else if(par.first == "argNum"){//@yuan_argIn:
                        int argIdx = std::stoi(par.second);
    ...
    for(auto node : _nodeindex){
        ...
        for(auto inwire : cell->inputWires()){
            ...
            if(srcCell->operation() == "CONST"){// don't record the edge from const node
                if(currentnode->hasImm(bitWidth)){//@yuan_argIn: for the node has two const
                    currentnode->set2ndImm(srcCell->Value());
                    currentnode->set2ndImmIdx(portIndex);
                }else{
