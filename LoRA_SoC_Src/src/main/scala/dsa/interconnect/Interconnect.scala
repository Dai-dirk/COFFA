...
//import fgramemfp.ppa.ppa_gib.getgibarea
...
//    connect.distinct.filter(ins => ! ((ins(0) <= 3 && trackDirections(ins(0)) == 0) || (ins(2) <= 3 && trackDirections(ins(2)) == 0)) )
//    connect.filter(ins => !(direction_reduce.contains(ins(0)) || direction_reduce.contains(ins(2))))
//    srcsList = srcsList.filter(ins => !(direction_reduce.contains(ins(0)) || direction_reduce.contains(ins(2))))
  }
  ...
  if(cfgsBit > 0) {
    ...
    connect.map { case (dst, srcs) =>
      if(srcs.size > 1) {
        ...
        offset += w
//        if(width == 1 ) println("mux size: " + srcs.size)
      }else{
        val src = srcs(0)
  ...
  connect.map{ case (dst, srcs) =>
    ...
    if(srcs.size > 1) {
      ...
      srcs.zipWithIndex.map { case (src, j) =>
        ...
        connections.append((smi_id("This")(0), "This", iPortMap(src_name), smi_id("Muxn")(i), "Muxn", j, width))
      }
      connections.append((smi_id("Muxn")(i), "Muxn", 0, smi_id("This")(0), "This", oPortMap(dst_name), width))
      // connect input configuration port to the Mux configuration ports
      val w = log2Ceil(srcs.size)
      ...
      val src = srcs(0)
      val src_name = portNameMap((src(0), src(1))) + src(2).toString
      connections.append((smi_id("This")(0), "This", iPortMap(src_name), smi_id("This")(0), "This", oPortMap(dst_name), width))
    }
  }
...
// object VerilogGen1 extends App {
//   // test getGIBConnect
...
////     i += 1
////   }
//   //numTrack :1  gib_par.diag_iopin_connect :true gib_par.num_iopin_list :Map(opin_ne -> 1, opin_nw -> 0, opin_sw -> 0, opin_se -> 1, ipin_ne -> 2, ipin_nw -> 0, ipin_sw -> 0, ipin_se -> 1) gib_par.connect_flexibility: Map(num_otrack_per_opin -> 2, num_itrack_per_ipin -> 2, num_ipin_per_opin -> 4)
////   Map(cfg_data_width -> 32, cfg_addr_width -> 12, num_track -> 1, y -> 0, data_width -> 32, diag_iopin_connect -> true, x -> 6, cfg_blk_offset -> 2, connect_flexibility -> Map(num_otrack_per_opin -> 2, num_itrack_per_ipin -> 2, num_ipin_per_opin -> 4), track_reged -> false, cfg_blk_index -> 19, num_iopin_list -> Map(opin_ne -> 1, opin_nw -> 0, opin_sw -> 0, opin_se -> 1, ipin_ne -> 2, ipin_nw -> 0, ipin_sw -> 0, ipin_se -> 1))
//
//   val num_iopin_list = mutable.Map("opin_ne" -> 1, "opin_nw" -> 0, "opin_sw" -> 0, "opin_se" -> 1, "ipin_ne" -> 2, "ipin_nw" -> 0, "ipin_sw" -> 0, "ipin_se" -> 1)
//   val connect_flexibility = mutable.Map(
//     "num_itrack_per_ipin" -> 2,
//     "num_otrack_per_opin" -> 2,
//     "num_ipin_per_opin"   -> 4
//   )
////   Map(num_otrack_per_opin -> 2, num_itrack_per_ipin -> 2, num_ipin_per_opin -> 4)
...
//     "track_directions, " -> ListBuffer(EAST, WEST, SOUTH, NORTH)
//   )
//
//   (new chisel3.stage.ChiselStage).emitVerilog(new GIB(attrs ),args)
////   getgibarea(1, true, num_iopin_list, connect_flexibility ,false,ListBuffer())
// }