...
class FarPath(val expWidth: Int, val precision: Int, val outPc: Int)
  ...
  val io = IO(new Bundle() {
    val in = Input(new Bundle() {
      val a, b = new RawFloat(expWidth, precision)//@yuan: b is always the smaller one
      val expDiff = UInt(expWidth.W)
      val effSub = Bool()//@yuan: eff_sub means two operands have different signs
      val smallAdd = Bool()
      val rm = UInt(3.W)
    })
  ...
  val adder_in_sig_b = Mux(effSub, (~adder_in_sig_b_raw).asUInt, adder_in_sig_b_raw) + effSub //@yuan: effSub = 1 means two operands have different signs, that is; a - b, so the mantissa of b should using complement code
   ...
   */
  //@yuan: maybe these two results are used when the adder result of mantissa overflow?
...
class NearPath(val expWidth: Int, val precision: Int, val outPc: Int)
  ...
  val b_sig = (Cat(b.sig, 0.U(1.W)) >> need_shift).asUInt//@yuan: only difference between exponent < 2^1 selects near path, that is, maximum shift bit here is 1
  ...
  val lza_ab = Module(new LZA(precision + 1))//@yuan: LZA is Leading Zero Anticipation
  lza_ab.io.a := a_sig
  lza_ab.io.b := b_neg
...
//  val exp_s2 = exp_s1 - lza_error
  ...
  result.exp := a.exp //near_path_exp
...
class FCMA_ADD_s1(val expWidth: Int, val precision: Int, val outPc: Int)
  ...
  val raw_a = RawFloat.fromFP(fp_a, Some(decode_a.expNotZero))//@yuan: some means here transmitting a concrete value (for option parameter)
  val raw_b = RawFloat.fromFP(fp_b, Some(decode_b.expNotZero))
  val eff_sub = raw_a.sign ^ raw_b.sign //@yuan: eff_sub = 1, mantissa requires subtraction; eff_sub = 0, mantissa requires addition; for mode = 1, the adder executes subtraction

  val small_add = decode_a.expIsZero && decode_b.expIsZero //@yuan: 2^0, only with mantissa
  ...
  val special_path_hasNaN = decode_a.isNaN || b_isNaN //@yuan: some flag signals, indicating that the operands are all zero, infinite or something wrong
  val special_path_hasSNaN = decode_a.isSNaN || b_isSNaN
  val special_path_hasInf = decode_a.isInf || b_isInf
  val special_path_inf_iv = decode_a.isInf && b_isInf && eff_sub //@yuan: a and b are +infinite/-infinite; iv means invalid?
  ...
  val exp_diff_a_b = Cat(0.U(1.W), raw_a.exp) - Cat(0.U(1.W), raw_b.exp)//@yuan: comparison between the exponents of a and b (for FP32, 9bit subtraction)
  ...
  val sel_far_path = !eff_sub || ea_minus_eb > 1.U || b_flags.overflow //@yuan: far path is used for two operands with different exponent
...
//println("far_path_inputs size: " + far_path_inputs.size)
  //@yuan: far path is used for two mantissas: (1) addition (eff_sub = 0); (2) subtraction (eff_sub = 1), but difference of exponents > 2^1
  val far_path_mods = far_path_inputs.map { in =>
    ...
    far_path.io.in.effSub := eff_sub //@yuan: eff_sub means two operands have different signs
    far_path.io.in.smallAdd := small_add
    far_path.io.in.rm := io.rm
    far_path
  }
  val far_path_out = far_path_mods.head.io.out//@yuan: to here, the mantissa haven't been added yet
...
//  println("near_path_inputs size: " + near_path_inputs.size)
  //@yuan: near path is used for: (1) subtraction, but difference of exponents <= 2^1
  val near_path_mods = near_path_inputs.map { in =>
    ...
    near_path.io.in.need_shift_b := in._3//@yuan: when the exponents are different, it requires shift b
  ...
  io.out.near_path_int_bit := near_path_out.int_bit
...
class FCMA_ADD_s1_to_s2(val expWidth: Int, val precision: Int, val outPc: Int)
  ...
  val far_sig_b = Output(UInt((precision+4).W))
  ...
  val near_path_int_bit = Output(Bool())
...
//    val fflags = Output(UInt(5.W))
  ...
  val cout = adder_result.head(1).asBool//@yuan: mantissa
  val keep = adder_result.head(2) === 1.U
  val cancellation = adder_result.head(2) === 0.U

  far_path_res.sig := Mux1H(//@yuan: Mux1H, one hot encode, only one select signal will be valid
  ...
  far_path_res.exp := Mux1H(//@yuan: select e+1; e; e-1, based on the addition result of mantissa
    Seq(cout, keep, cancellation),
    io.in.far_exp_a_vec
  )
  ...
  val exp_s1 = io.in.near_path_out.exp - lzc
  val exp_s2 = exp_s1 - io.in.near_path_lza_error
  near_path_res.exp := Mux(io.in.near_path_int_bit, exp_s2, 0.U)
...
class FCMA_ADD_1(val expWidth: Int, val precision: Int, val outPc: Int)
  ...
  val ea_minus_eb = Mux(need_swap, exp_diff_b_a.tail(1), exp_diff_a_b.tail(1))
  val sel_far_path = !eff_sub || ea_minus_eb > 1.U || b_flags.overflow //@yuan: only the difference of exponents are > 2^1 (that is ,minimum is 2^2), selecting far path
...
//    val fflags = Output(UInt(5.W))
  ...
  fadd_s1.io.a := io.a
  fadd_s1.io.b := Cat(io.b.head(1)^io.mode, io.b.tail(1))
...
//@yuan: mode is used to indicate addition or subtraction
...
//    val fflags = Output(UInt(5.W))
  ...
  fcmaadd.io.b := io.b
  fcmaadd.io.rm := io.rm
  fcmaadd.io.mode := io.mode
  fcmaadd.io.b_inter_valid := false.B //@yuan: for FCMA, the FMUL transmits the control signals
  fcmaadd.io.b_inter_flags := DontCare
  io.result := fcmaadd.io.result
