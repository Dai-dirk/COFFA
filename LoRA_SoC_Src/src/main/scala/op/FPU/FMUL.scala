...
class FMUL_share(val expWidth: Int, val precision: Int) extends Module {
  val io = IO(new Bundle() {
    val a, b = Input(UInt((expWidth + precision).W))
    val sigProduct = Input(UInt((2*precision).W))
    val rm = Input(UInt(3.W))
    val result = Output(UInt((expWidth + precision).W))
    val a_sig, b_sig = Output(UInt(precision.W))
//    val fflags = Output(UInt(5.W))
//    val to_fadd = Output(new FMULToFADD(expWidth, precision))
  })
...
//  multiplier.io.regEnables.foreach(_ := true.B)
  ...
  fmul_s2.io.prod := io.sigProduct

  fmul_s3.io.in := RegNext(fmul_s2.io.out)

//  io.to_fadd := fmul_s3.io.to_fadd
//  io.result := fmul_s3.io.result
//  io.fflags := fmul_s3.io.fflags
//    io.to_fadd := RegNext(fmul_s3.io.to_fadd)
    io.result := RegNext(fmul_s3.io.result)
//    io.fflags := RegNext(fmul_s3.io.fflags)
...
class FMUL(val expWidth: Int, val precision: Int) extends Module {
  ...
  fmul_s2.io.in := fmul_s1.io.out
  fmul_s2.io.prod := raw_a.sig * raw_b.sig
...
////  multiplier.io.regEnables.foreach(_ := true.B)
//
//  fmul_s1.io.a := RegNext(io.a)
//  fmul_s1.io.b := RegNext(io.b)
//  fmul_s1.io.rm := RegNext(io.rm)
//
//  fmul_s2.io.in := fmul_s1.io.out
...
object VerilogFMULGen extends App {
  (new chisel3.stage.ChiselStage).emitVerilog(new FMUL(6, 10), args)
}