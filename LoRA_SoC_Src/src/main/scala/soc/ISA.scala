...
//  val CGRA_EXE_CMD = 4.U
  ...
  val LD_DEP_ST_LAST_TASK = 3.U     // this load command depends on the store command of last task
  val LD_DEP_EX_LAST_TASK = 2.U     // this load command depends on the execute command of last task
  val LD_DEP_ST_LAST_SEC_TASK = 1.U // this load command depends on the store command of last second task
  ...
  val LOAD_CMD_RS2_TOTAL_LEN_WIDTH = 16
  val LOAD_CMD_RS2_PARTITION_SIZE_WIDTH = 2
  val LOAD_CMD_RS2_PARTITION_BLOCK_SIZE_WIDTH = 4
//  val LOAD_CMD_RS2_PARTITION_MODE_WIDTH = 1
  // val LOAD_CMD_RS2_UNUSE0_WIDTH = 1
  class LoadRs2 extends Bundle {
    ...
    val lgPartitionSize = UInt(LOAD_CMD_RS2_PARTITION_SIZE_WIDTH.W) //@yuan: the number of partitioned memory in log format 
    val lgPartitionBlockSize = UInt(LOAD_CMD_RS2_PARTITION_BLOCK_SIZE_WIDTH.W) //@yuan: the size of partitioned block in log format (i.e. log(B))
//    val partition_mode = UInt(LOAD_CMD_RS2_PARTITION_MODE_WIDTH.W) //@yuan: the partition mode: 0: normal mode, 1: partition mode
  ...
  val STORE_CMD_RS2_TOTAL_LEN_WIDTH = 16
  val STORE_CMD_RS2_PARTITION_SIZE_WIDTH = 2
//  val STORE_CMD_RS2_PARTITION_MODE_WIDTH = 1
  val STORE_CMD_RS2_PARTITION_BLOCK_SIZE_WIDTH = 4
  val STORE_CMD_RS2_UNUSE0_WIDTH = 1
  class StoreRs2 extends Bundle {
    val dep_type = UInt(DEP_TYPE_WIDTH.W) // unused
    val task_id = UInt(TASK_ID_WIDTH.W)
    val lgPartitionSize = UInt(STORE_CMD_RS2_PARTITION_SIZE_WIDTH.W) //@yuan: the number of partitioned memory in log format 
    val lgPartitionBlockSize = UInt(STORE_CMD_RS2_PARTITION_BLOCK_SIZE_WIDTH.W) //@yuan: the size of partitioned block in log format (i.e. log(B))
//    val partition_mode = UInt(STORE_CMD_RS2_PARTITION_MODE_WIDTH.W) //@yuan: the partition mode: 0: normal mode, 1: partition mode
    val _unuse0 = UInt(STORE_CMD_RS2_UNUSE0_WIDTH.W)
    val fused = UInt(1.W) // can be fused with the next load command, with the same remote_addr, total_len
    val total_len = UInt(STORE_CMD_RS2_TOTAL_LEN_WIDTH.W)
    val spad_addr = UInt(STORE_CMD_RS2_SPAD_ADDR_WIDTH.W)
  }
