...
class ReservationStation(loadQueDepth: Int, storeQueDepth: Int, exeQueDepth: Int, idWidth: Int)
  ...
  val taskIdWidth = FgramemISA.TASK_ID_WIDTH
  assert(idWidth > taskIdWidth)

  val cmd_que = Module(new Queue(new RoCCCommand, 4))
...
//      }
  ...
  // command enque
  ld_cmd_que.io.enq.valid := cmd.fire && isLoadCmd
  ld_cmd_que.io.enq.bits  := cmd.bits
  ex_cmd_que.io.enq.valid := cmd.fire && (isCfgCmd || isExeCmd)
  ex_cmd_que.io.enq.bits  := cmd.bits
  st_cmd_que.io.enq.valid := cmd.fire && isStoreCmd
  st_cmd_que.io.enq.bits  := cmd.bits
