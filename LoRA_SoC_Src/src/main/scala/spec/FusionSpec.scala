...
// Architecture Specification
 ...
 class GpeSpec(// num_reg_rf_for_alu_ : Int = 1, num_reg_rf_for_lut_ : Int = 1,
               ...
               gpe_mode_ : Int = 0,//@hw_yuan: mode=0: ACC/MAC; mode=1: Unified Unit
...
//   var num_reg_rf_for_lut =  num_reg_rf_for_lut_  // number of registers in Regfile for LUT
   ...
   var gpe_mode = gpe_mode_ //@hw_yuan: mode=0: ACC/MAC; mode=1: Unified Unit; mode = 2: LNS Unit
...
//   var to_dir = todir_            // which directions the GPE outputs are to
  ...
  object GpeSpec {
   def apply(max_delay_cg_ : Int = 4,
             ...
             gpe_mode_ : Int = 0,
             operations_ : ListBuffer[String] = ListBuffer( "PASS", "ADD", "SUB")) = {
     new GpeSpec(max_delay_cg_, max_delay_fg_, num_input_lut_, gpe_mode_, operations_)
...
//       num_input_lut_, operations_, fromdir_, todir_)
 ...
 object FusionSpec{
   ...
   fgramemfp.common.CompileMacroVar.USE_SRAM_BLACKBOX = USE_SRAM_BLACKBOX
...
//   val fgra_cfg_sram_banks_cascade = 2 // cascading sram banks that FGRA config controller can access
   ...
   val attrs: mutable.Map[String, Any] = mutable.Map(
     ...
     "fgra_gpe_mode" -> 0, //@hw_yuan: the mode of GPE
   ...
   def setDefaultGpesSpec(): Unit = {
     ...
     for(i <- 0 until attrs("fgra_num_row").asInstanceOf[Int]){
       ...
       for( j <- 0 until attrs("fgra_num_colum").asInstanceOf[Int]){
         ...
         val gpe_mode = attrs("fgra_gpe_mode").asInstanceOf[Int]
         ...
         gpes_spec(i).append(GpeSpec(max_delay_cg, max_delay_fg, num_input_lut, gpe_mode, operations))
       }
     }
         ...
         if (kv._1 == "fgra_gpe_operations") {
           ...
           for (i <- gpe_2d.indices) {
             ...
             for (j <- gpe_1d.indices) {
               ...
               var gpe_mode : Int = 0
               if(gpemap.contains("gpe_mode")) {
                 gpe_mode = gpemap("gpe_mode").asInstanceOf[Int]
               }
//               val operations = ListBuffer[String]() ++ gpemap("operations").asInstanceOf[List[String]]
               val operations: ListBuffer[String] = {
                 if(gpe_mode < 2) {
                   ...
                   gpemap("operations").asInstanceOf[List[String]].foreach { op =>
                     if (op == "ARITH") {
                       ops ++= ListBuffer("ADD", "SUB", "ULE", "ULT", "UGE", "UGT", "SLT", "SGE", "SGT", "SLE")
                     } else if (op == "LOGIC") {
                       ops ++= ListBuffer("AND", "OR", "NOT", "XOR", "XNOR", "NE", "EQ")
                     } else if (op == "SHIFT") {
                       ops ++= ListBuffer("SHL", "LSHR", "ASHR", "CSHL", "CSHR")
                     } else if (op == "FEAS") { //@yuan_fp
                       ops ++= ListBuffer("FEXADD", "FEXSUB")
                     } else if (op == "FCMP") { //@yuan_fp
                       ops ++= ListBuffer("FEQ", "FLT", "FLE")
                     } else if (op == "FADDSUB") { //@yuan_fp
                       ops ++= ListBuffer("FADD", "FSUB")
                     } else {
                       ops ++= ListBuffer(op)
                     }
                   }
                   //@yuan: when the PE has LUT, it can support logic operations
                   if(num_input_lut > 0){
                      ops ++= ListBuffer("AND", "OR", "NOT", "XOR", "XNOR")
                   }
                   ops
                 }else{
                   ListBuffer("XCORE")
