...
const DATA_TYPE beta1 = 3, beta2 = 1, const1 = 1, const2 = 1;     // 1.0 -2.0 beta1: 1/ beta beta2: beta

#define MAT_DIM_I 32
#define MAT_DIM_K 32
#define MAT_DIM_J 32
const DATA_TYPE beta = 3;      // 0.1 0.0 1.0
...
inline DATA_TYPE my_exp(DATA_TYPE x) {
    ...
    DATA_TYPE exp_frac_x  = 1.0f + frac_x * (1.0f + frac_x * (0.5f));
...
inline DATA_TYPE my_log(DATA_TYPE x) {
    ...
    DATA_TYPE log_mantissa = frac_x * (1.0f + frac_x * (-0.5f));
...
void kernel()
{
    ...
    {	
        for (int j = 0; j < MAT_DIM_J; ++j)
        {
            ...
            igelu_C[i][j] = sum * ((DATA_TYPE)(const1) - exp_2x) / ((DATA_TYPE)(const1) + exp_2x);
        }
    }
#ifdef CGRA_COMPILER
    loop_end();
