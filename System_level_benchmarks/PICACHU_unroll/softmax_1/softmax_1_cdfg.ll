...
entry:
  %0 = getelementptr inbounds [1 x float], [1 x float]* @max_arr, i32 0, i32 0
  %1 = load volatile float, float* %0, align 4, !tbaa !3
  br label %for.body

for.cond.cleanup:                                 ; preds = %for.body
  %maxx.1.1.lcssa = phi float [ %maxx.1.1, %for.body ]
  %2 = getelementptr inbounds [1 x float], [1 x float]* @max_arr, i32 0, i32 0
  store volatile float %maxx.1.1.lcssa, float* %2, align 4, !tbaa !3
  ret void

for.body:                                         ; preds = %for.body, %entry
  %i.012 = phi i32 [ 0, %entry ], [ %inc.1, %for.body ]
  %maxx.011 = phi float [ %1, %entry ], [ %maxx.1.1, %for.body ]
  %arrayidx = getelementptr inbounds float, float* %input, i32 %i.012
  %3 = load float, float* %arrayidx, align 4, !tbaa !3
  %cmp1 = fcmp ogt float %3, %maxx.011
  %maxx.1 = select i1 %cmp1, float %3, float %maxx.011
  %inc = or i32 %i.012, 1
  %arrayidx.1 = getelementptr inbounds float, float* %input, i32 %inc
  %4 = load float, float* %arrayidx.1, align 4, !tbaa !3
  %cmp1.1 = fcmp ogt float %4, %maxx.1
  %maxx.1.1 = select i1 %cmp1.1, float %4, float %maxx.1
