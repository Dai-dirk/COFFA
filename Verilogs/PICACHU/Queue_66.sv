...
module Queue_66(
  ...
  output       io_enq_ready,
               io_deq_valid,
  output [4:0] io_deq_bits_min_pos,
               io_deq_bits_max_pos,
  output [7:0] io_deq_bits_id,
  output       io_deq_bits_exception
);

  wire [18:0] _ram_ext_R0_data;	// @[Decoupled.scala:273:95]
  ...
  ram_combMem_9 ram_ext (	// @[Decoupled.scala:273:95]
    ...
    .W0_clk  (clock),
    .W0_data ({io_enq_bits_exception, io_enq_bits_id, io_enq_bits_max_pos, io_enq_bits_min_pos}),	// @[Decoupled.scala:273:95]
    .R0_data (_ram_ext_R0_data)
  );
  assign io_enq_ready = ~full;	// @[Decoupled.scala:279:24, :303:19]
  assign io_deq_valid = ~empty;	// @[Decoupled.scala:278:25, :302:19]
  assign io_deq_bits_min_pos = _ram_ext_R0_data[4:0];	// @[Decoupled.scala:273:95]
  assign io_deq_bits_max_pos = _ram_ext_R0_data[9:5];	// @[Decoupled.scala:273:95]
  assign io_deq_bits_id = _ram_ext_R0_data[17:10];	// @[Decoupled.scala:273:95]
  assign io_deq_bits_exception = _ram_ext_R0_data[18];	// @[Decoupled.scala:273:95]
endmodule
